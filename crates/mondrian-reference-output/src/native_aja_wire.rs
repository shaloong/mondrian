...
#[derive(Debug, Clone)]
pub struct AjaWireReadbackConfiguration {
    ...
    /// Exact SDI signal physically wired from output SDI1 to receiver SDI1.
...
impl AjaWireReadbackConfiguration {
    pub(super) fn serial(&self) -> Result<u64, ReferenceOutputAdapterError> {
        let raw = self
            .device_id
            .as_str()
            .strip_prefix("aja:")
            .and_then(|id| id.strip_suffix(":sdi1"))
            .ok_or_else(|| vendor("wire-plan", "receiver must be an exact AJA SDI1 identity"))?;
    ...
    pub(super) fn preflight(
        ...
        if device.serial == output_serial
    ...
    pub(super) fn open(
        ...
        for _ in 0..1024 {
            ...
            let path = configuration
                .receipt_directory
                .join(format!("aja-wire-{nonce}-{serial:08}.jsonl"));
    ...
    fn drop(&mut self) {
        ...
        let spawn =
            std::thread::Builder::new()
                .name("aja-abandoned-capture".to_owned())
                .spawn(move || {
