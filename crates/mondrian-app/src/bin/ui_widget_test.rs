...
impl Widget for RootFill {
    ...
    fn measure(&self, c: LayoutConstraint) -> Size { c.constrain(Size::new(200.0, 100.0)) }
    fn layout(&mut self, bounds: Rect) {
        ...
        if let Some(c) = self.children.get_mut(1) {
            c.layout(Rect::new(bounds.x + half_w, bounds.y, half_w, bounds.height));
        }
    }
    ...
    fn paint(&self, ctx: &mut PaintContext) {
        ...
        eprintln!("PAINT RootFill bg bounds=({:.0},{:.0} {:.0}x{:.0})", self.bounds.x, self.bounds.y, self.bounds.width, self.bounds.height);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut root = RootFill::new(vec![
        Box::new(ColoredBox::new(Color { r: 1.0, g: 0.0, b: 0.0, a: 1.0 }, 300.0, 400.0).with_label("red")),
        Box::new(ColoredBox::new(Color { r: 0.0, g: 0.0, b: 1.0, a: 1.0 }, 300.0, 400.0).with_label("blue")),
    ]);
    ...
    el.run(move |ev, elwt| {
        ...
        match ev {
            ...
            Event::WindowEvent { event: WindowEvent::Resized(ns), .. } => {
                if ns.width > 0 && ns.height > 0 { cfg.width = ns.width; cfg.height = ns.height; surf.configure(&dev, &cfg); }
            }
