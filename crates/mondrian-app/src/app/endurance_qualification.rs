...
struct EnduranceProducerRawEvidence<'a> {
    ...
    #[serde(flatten)]
    ancillary: Option<&'a mondrian_platform::EnduranceAncillaryPhaseEvidence>,
    schema_version: u32,
    phase_id: &'a str,
    run_id: &'a str,
...
pub struct EndurancePhaseCapture {
    measurement_timing: Option<mondrian_platform::EndurancePhaseMeasurementTiming>,
    ancillary: Option<mondrian_platform::EnduranceAncillaryPhaseEvidence>,
    ancillary_export_events: Vec<mondrian_platform::EnduranceAncillaryExportArtifact>,
...
impl EndurancePhaseCapture {
    fn new(
        ...
        Ok(Self {
            ...
            ancillary: None,
            ancillary_export_events: Vec::new(),
            measurement_timing: None,
        })
    }

    pub(crate) fn record_ancillary_export_artifact(
        &mut self,
        artifact_id: &str,
        receipt: Option<mondrian_platform::EnduranceAncillaryExportArtifact>,
    ) -> Result<(), EnduranceCaptureError> {
        if let Some(receipt) = receipt {
            if receipt.artifact_id != artifact_id || self.ancillary_export_events.len() >= 256 {
                return Err(EnduranceCaptureError::InvalidProducerEvidence);
            }
            self.ancillary_export_events.push(receipt);
        }
        Ok(())
    }

    /// Freeze the measurement origin once, before the first sample or event.
    pub(crate) fn begin_measurement(
        &mut self,
        timing: mondrian_platform::EndurancePhaseMeasurementTiming,
    ) -> Result<(), EnduranceCaptureError> {
        if self.measurement_timing.is_some()
            || self.last_sample.is_some()
            || !timing.validates()
            || timing.startup_started_at_run_us != self.started_at_run_us
            || timing
                .measurement_deadline_at_run_us
                .checked_sub(timing.measurement_started_at_run_us)
                != Some(self.requirement.minimum_duration_us)
        {
            return Err(EnduranceCaptureError::InvalidProducerEvidence);
        }
        self.started_at_run_us = timing.measurement_started_at_run_us;
        self.measurement_timing = Some(timing);
        Ok(())
    }

    pub(crate) fn bind_ancillary_evidence(
        &mut self,
        evidence: Option<mondrian_platform::EnduranceAncillaryPhaseEvidence>,
    ) -> Result<(), EnduranceCaptureError> {
        let actual_artifact_ids = self
            .semantic
            .events
            .iter()
            .filter_map(|event| match event {
                EnduranceProducerEvent::ExportArtifactVerified { artifact_id, .. } => {
                    Some(artifact_id)
                }
                _ => None,
            })
        ...
        match &evidence {
            Some(evidence)
                if evidence.validates_inventory()
                    && evidence.ancillary_export_artifacts == self.ancillary_export_events
                    && evidence
                        .ancillary_export_artifacts
                        .iter()
                        .map(|receipt| &receipt.artifact_id)
                        .eq(actual_artifact_ids)
                    && (self.requirement.kind == EndurancePhaseKind::ContinuousExport
                        || !evidence.wire_journals.is_empty()) => {}
            None if self.ancillary_export_events.is_empty() => {}
            _ => return Err(EnduranceCaptureError::InvalidProducerEvidence),
        }
        self.ancillary = evidence;
        Ok(())
    }
    ...
    fn publish_producer_evidence(
        ...
        let raw = EnduranceProducerRawEvidence {
            measurement_timing: self.measurement_timing,
            ancillary: self.ancillary.as_ref(),
            schema_version: 2,
...
impl EnduranceRunCapture {
    ...
    pub fn seal_manifest(
        ...
        self.seal_manifest_with_owner_history(output_path, owner_closure, Vec::new())
    }

    /// Publish the final manifest with the owner's already durable complete phase history.
    pub fn seal_manifest_with_owner_history(
        self,
        output_path: &Path,
        owner_closure: EnduranceRunOwnerClosureEvidence,
...
pub(crate) fn write_json_create_new(
    path: &Path,
...
mod tests {
    ...
    #[test]
    fn ancillary_owner_inventory_requires_every_actual_export_event_in_order() {
        ...
        let mut capture = EndurancePhaseCapture {
            measurement_timing: None,
            chunks: EnduranceChunkRecorder::new(&requirement.phase_id, 1, 1).expect("chunks"),
            requirement,
            run_id: "test-run".to_owned(),
            authority_challenge: "test-challenge".to_owned(),
            started_at_run_us: 0,
            evidence_directory: PathBuf::new(),
            receipts: Vec::new(),
            last_sample: None,
            all_reference_samples_hardware_backed: true,
            semantic: EnduranceSemanticRecorder::new(EndurancePhaseKind::ContinuousExport, 4),
            ancillary: None,
            ancillary_export_events: Vec::new(),
        };
        let receipts = (0..2)
            .map(
                |index| mondrian_platform::EnduranceAncillaryExportArtifact {
        ...
        for (index, receipt) in receipts.iter().enumerate() {
            capture
                .record_export_artifact_verified(
                    index as u64,
                    &receipt.artifact_id,
                    SHA,
                    "validator",
                ...
                .expect("actual event");
            capture
                .record_ancillary_export_artifact(&receipt.artifact_id, Some(receipt.clone()))
                .expect("attached owner receipt");
        }
        let evidence = mondrian_platform::EnduranceAncillaryPhaseEvidence {
            ancillary_program_sha256: SHA.to_owned(),
            ancillary_export_artifacts: receipts.clone(),
            wire_journals: Vec::new(),
        };
        assert!(capture.bind_ancillary_evidence(None).is_err());
        let mut missing = evidence.clone();
        missing.ancillary_export_artifacts.pop();
        assert!(capture.bind_ancillary_evidence(Some(missing)).is_err());
        let mut reordered = evidence.clone();
        reordered.ancillary_export_artifacts.reverse();
        assert!(capture.bind_ancillary_evidence(Some(reordered)).is_err());
        assert!(capture.bind_ancillary_evidence(Some(evidence)).is_ok());
        assert!(capture
            .record_ancillary_export_artifact("wrong-export", Some(receipts[0].clone()))
            .is_err());
    }
    ...
    fn run_capture_publishes_chunks_and_manifest_without_hand_authored_hashes() {
        ...
        let mut measurement_probe = capture
            .begin_phase(
                "01-playback-reference-24h",
                0,
                &evidence,
            ...
            .expect("measurement activation contract probe");
        let timing = mondrian_platform::EndurancePhaseMeasurementTiming {
            startup_started_at_run_us: 0,
            startup_deadline_at_run_us: 120_000_000,
            owners_ready_at_run_us: 9_000_000,
            measurement_started_at_run_us: 9_000_000,
            measurement_deadline_at_run_us: 9_000_000
                + measurement_probe.requirement.minimum_duration_us,
        };
        let mut expired = timing;
        expired.startup_deadline_at_run_us = timing.measurement_started_at_run_us;
        assert!(measurement_probe.begin_measurement(expired).is_err());
        assert_eq!(measurement_probe.started_at_run_us, 0);
        assert!(measurement_probe.measurement_timing.is_none());
        measurement_probe.begin_measurement(timing).expect("first activation");
        assert!(measurement_probe.begin_measurement(timing).is_err());
        assert_eq!(measurement_probe.measurement_timing, Some(timing));
        assert_eq!(measurement_probe.started_at_run_us, 9_000_000);
