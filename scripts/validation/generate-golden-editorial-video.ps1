...
$FixtureId = "generated-golden-rec709-h264-25fps-v1"
$FileName = "golden-rec709-h264-25fps-v1.mp4"
$RecipePath = "scripts/validation/generate-golden-editorial-video.ps1"
...
function Assert-Probe([object]$Probe) {
    ...
    if ($video.Count -ne 1 -or @($Probe.streams).Count -ne 1) {
        throw "Generated Golden editorial fixture must contain exactly one H.264 video stream."
    }
    $stream = $video[0]
    if (
        ...
        throw "Generated Golden editorial fixture does not satisfy the H.264 High Rec.709 contract."
    }
    if ([double]$Probe.format.duration -lt 7.9) {
        throw "Generated Golden editorial fixture is shorter than 7.9 seconds."
    }
}
...
function Assert-ExistingAttestation([string]$ArtifactPath, [string]$RecipeHash) {
    ...
    if (-not (Test-Path -LiteralPath $attestationPath -PathType Leaf)) {
        throw "Existing Golden editorial fixture has no attestation. Re-run with -Force."
    }
    ...
    if (
        ...
        throw "Existing Golden editorial fixture is not attested for the current recipe and artifact. Re-run with -Force."
    }
}
...
if ($encoderList -notmatch "\blibx264\b") {
    throw "The Golden editorial recipe requires the FFmpeg libx264 encoder."
}
$filterList = ((& $ffmpeg -hide_banner -filters 2>&1) -join [Environment]::NewLine)
if ($filterList -notmatch "\btestsrc2\b") {
    throw "The Golden editorial recipe requires FFmpeg testsrc2 support."
}
if ($ValidatePrerequisitesOnly) {
    Write-Host "Golden editorial video generator prerequisites are available."
    return
}
