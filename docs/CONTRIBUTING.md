...
- 三个平台都必须在净化环境中执行 `mondrian --verify-runtime`；该命令拒绝 PATH-only 工具，并验证链接 decoder、CLI encoder/filter/muxer 和 `--enable-nonfree`。Windows 验证时 `PATH` 只保留分发目录与系统目录，vcpkg 构建树不得补齐漏打包 DLL；Linux 会拒绝解析到包外的非基础 ELF 依赖；macOS 会检查主程序、两个工具及每个内嵌 Framework image 的全部非系统加载边。媒体或 OCIO 运行时不完整会直接阻止发布产物上传。
