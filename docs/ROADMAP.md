...
| 效果与动画 | 稳定 `EffectId` 与 `ParameterId`、实例地址、版本化 `ParameterSchema`、定义默认值/可动画能力、受约束数值/enum/resource、三种执行插值语义、`PropertyBag`/`AnimatedProperty`、编辑器曲线预设、效果 DAG、mask、缓存策略和插件式 definition/DSL 已存在；视觉与音频 Processor 共用同一参数描述语言，项目加载会拒绝非法参数状态，内建执行按 ParameterId 精确寻址；Mask 标量 Property Bag 自 schema v17 起持久化，Basic Title 封闭 Property Bag/精确动画自 schema v18 起持久化；Cross Dissolve 已进入共享 Preview/Export working-linear coverage-correct CPU/GPU 合成并有产品时间线手势；Basic Title 已贯通创建、Inspector、Undo、共享 Render Plan、后台 Preview、Export、嵌套、转场端点与缺失字体 fail-closed，断开的 `TextLayer` 已删除；未知插件定义失败关闭；效果库只列出有可执行图的定义 | 只有部分声明效果生成真实 render op；CPU/GPU backend 与真实 preview/export 验证仍须分别声明；Cross Dissolve 与 Basic Title 仍须 Golden Project 和外部参考视觉验收；真实外部插件仍需 Adapter | L1 地基，产品广度未完成 |
...
- [ ] Hold、Linear、Bezier/Ease、关键帧增删移动复制、reset 和基础 curve editor 可完成 Golden Project。当前视觉 Golden 切片已证明 Hold/Linear/Bezier 两关键帧的正式作者接口、求值、单步 Undo/Redo、schema 保存重开和 Preview/Export 同义；尚未证明 Ease 预设、稳定选择、增删/移动/复制/reset 的完整产品 UI。现有简化 Curve Editor 仍可能把非 Linear 插值重写为 Linear，必须先改为按稳定 KeyframeId/ParameterId 发出细粒度 mutation，不能因内核门禁通过而勾选。
