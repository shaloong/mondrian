...
- [ ] 为 Clip/Track/Bus/Program Output 的 Rack、参数自动化和 meter 提供同一套稳定 Scope/Processor/ParameterId 驱动的编辑 UI/命令。共享作者地基已使用 `AudioProcessorRackEditRequest` 封闭 Rack 地址与编辑代数：Scope 或类型化 Channel Strip/Pre/Post 地址、稳定相对 placement、Processor/Parameter/Keyframe ID、完整 Audio Program 原子校验、单次事务、无操作零提交及 Undo/Redo 均有跨 Timeline/App 证据；保存重开继续由 current document v23 的 Processor Schema/精确曲线回归覆盖。Timeline 以 `inspect_audio_processor_rack` 同时作为 Rack 解析、全 Sequence Scope 绑定统计和 Track 锁准入的唯一只读 Interface；App UI 不再扫描作者图复制规则，非法地址形成显式禁用诊断。规范 Channel Strip 又以独立 `AudioChannelStripEditRequest` 承载 Track/Bus/Output input trim、静态 fader 和精确 Sequence-time fader key 操作；地址错误、Track 锁 blocker 与非法 mutation 分型，事务在 COW detach 前准入、整图校验后提交，自动化存在时静态 fader 明确拒绝，删除末键规范回静态值。音频工作区已有稳定 `Mixer` Panel：按 Track→Bus→Program Output 投影 input trim、静态或自动化 fader、Track mute 以及共用 Pre/Post Rack；Inspector 与 Mixer 复用同一 Rack 控件 Implementation。Clip Processing Scope Inspector 继续按 Scope 去重，产品目录在 dispatch 时创建规范 Gain/Limiter 实例；稳定 ID reorder/remove/bypass 和无关键帧参数编辑走同一事务，已有关键帧只显示计数而不冒充播放头值。首个 lookahead limiter 已通过 state、entry/deadline/PDC、播放/导出一致性、callback 实时安全和多轨负载矩阵。仍缺精确 owner-time curve editor、route/send 与 Bus 创建产品流程、meter/solo session overlay、UI 交互 Golden、外部插件 Adapter 与真实设备长时证据，因此本项不提前勾选。
