...
`GoldenProductWorkflowDriver` 仍持有一个真实 App composition、固定 `ProjectId + project path`，并使七个切片可复用为诊断阶段；其私有 Hero binding 锁定初始 `SequenceId`、完整设置和 Project Color Environment，Hero slice 只能复用该身份，诊断 slice 才能通过产品作者事务创建新 Sequence。Foundation Audio 与 Visual Authoring 现共享 Hero，其余阶段连同 Recovery/Nesting child 仍形成六条诊断 Sequence，当前完整 Project 共七条。此前八条隔离 Sequence 的三轮运行真实证明了局部路径和共同 Project 生命周期，但没有证明它们在一条 5 分钟节目时间线上共同成立，因此该三轮不再计作 M1 完整 Golden。v7 会在重载荷执行前明确列出剩余 Hero 缺失义务并失败关闭。下一步不是删除切片，而是让素材编辑、颜色、嵌套和交付逐步复用该 Hero Sequence；最终 durable reopen 必须保留身份、7,500 帧内容边界及全部关联证据。
