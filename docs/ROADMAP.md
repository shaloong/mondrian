...
| 播放与缓存 | UI 无关 `PlaybackEngine`、`FrameWorkBroker`、`PreviewFrameStore`、三种访问语义、FFmpeg Session/isolated helper、generation/cancellation/deadline 和 Window/Headless production runtime 已接入。解码选择以 `DecodedTemporalExtent` 的半开 `[start,end)` 为权威：正 duration 与首个 successor 共同收紧边界，跨 forward call 最多保留 selected + successor 两个候选。Playback/Still 只接受因果覆盖请求的帧，否则失败关闭；Scrub 才可把非覆盖近邻标为 Explicitly degraded。取消记录 request→checkpoint→return，旧 generation 不得发布。单一 `viewer_gpu_device_progress` 非 UI worker 驱动 exact submission/callback；替换 Window 时同一 device generation、retained owner 与 cleanup authority 一并 handoff，迟到 callback 只能 retire 旧提交。当前提交 `0175f35` 的正式 4K Video 单门禁已在合格 16 GiB Windows 参考机自然完成 30 分钟/45,002 帧观察、50 次 warm seek、52 次 accurate seek、100 次 supersession、exact GPU completion、取消、helper、进程树内存与 post-stress 收敛，全部结构化合同通过 | 当前工作树仍须完成同一计划的真实 CPAL/A/V 音频门禁并以完整 Video+Audio gate set 取得 baseline；还须补 Window replacement/handoff、多 GPU/驱动/音频设备、真实阻塞媒体、8 GiB 降级和声学 loopback。单门禁虽绑定 clean revision 和合格机器，但只能是 `passed-diagnostic`，不能冒充完整播放 baseline | L1 代码契约与当前 Windows 长时视频门禁闭合；完整 A/V、设备和 Window handoff 资格未完成 |
...
- [ ] `cpal_av_48khz_30min_recovery_v2` 的真实墙钟门禁要求主音频流自身覆盖观察区间，不能用容器时长或 EOF 补零；必须先稳定观察一个具体 48 kHz stereo CPAL generation，再精确请求一次 validation-only 真实 stream recycle，证明真实流销毁后的 Audio→Synthetic 切换、新 generation 的 hidden-preroll/exact-trim、带不确定度的 Synthetic→Audio 相位交接，以及最终 generation 连续 30 分钟的 callback consumption。同期必须有 Headless GPU presentation、Ready ≥99.5%、恢复稳定后零额外 loss/underrun/recovery/静默替代、可证明的 A/V phase error ≤20 ms、Session resident/peak 有界且实际顺序复用、十秒稳态窗口 ≤460 ms、源缓存 ≤256 MiB 与完整产品进程树内存收敛（包含持久 FFmpeg 音频子进程）。当前诊断已发现并修复真实 WASAPI 首 callback 的位置资格漏洞：512 个 active-consumed frames 小于 590 帧剩余 playback delay 时，Adapter 现在保持 `Uncertain`/Synthetic，而非发布会在 Engine 中变成负位置的 `Running` 证据。修复后的既有 30 分钟运行中，两代真实 stream、Audio→Synthetic→Audio、1,801.077 秒 Audio Device 驻留、零 underrun/替代/recovery、A/V point-error p95 1.714 ms、缓存和进程树内存均通过；当时 Headless Ready 为 99.32%（53,585/53,949，364 Stale），未达 99.5%。此后 Viewer Runtime 已加入 Window/Headless 共用的 capacity-one immediate-successor：它以 Epoch/quality revision/精确 frame 与像素 output identity 两个独立维度绑定 ticketless 预备结果，支持相同像素的 already-visible alias、不同普通 GPU 输出的独立物理 lease 和显式 Transparent；generation/seek/quality 变化同步撤销，只有精确当前坐标可晋升，异构 Effect 尚未冒充已支持预备。current/prepared 的物理所有权现也已收敛到唯一 UI 无关 Module：Window/Headless 只能附加各自 artifact，exact promotion、submission-scoped retirement 与 move-only lease 规则不再重复；device-generation failure、非 GPU current、Viewer 几何变化或语义/物理不一致都必须返回并清理两个槽，不得静默遗留 prepared 语义。2026-08-06 的最新 3,600 帧 Release CPAL/GPU 诊断逐帧推进且预备覆盖 3,600/3,600，Ready 3,586/3,600（99.611%）、14 个孤立 `DroppedLate`、最大连续 1、零 underrun；14 次均在候选尝试后发生，保守 A/V proven phase error 最大 12.471 ms，超过当前 12 ms presentation-phase 上限，未被重标为 Ready。所有 coordinator stage 最大 2.564 ms（candidate 0.164 ms），GPU completion/物理 ownership 闭合，完整产品进程树峰值 Private Commit 约 634 MiB。统一 physical-publication 后又以本地临时 WAV 运行 300 帧 Release 真实 CPAL+Headless GPU 复验：successor preparation 300/300、Ready interval 299/300、单个 `DroppedLate`、最大连续 Stale 1、零 rejected/underrun，Audio Device point-error p95 0.608 ms，完整进程树峰值 Private Commit 617,340,928 bytes；该画面命中 already-visible alias，因此只证明真实协调路径无回归，distinct prepared 物理 lease 的所有权由 Module 单测和 Viewer GPU 生命周期回归证明，不能拿这次短门禁冒充其真实像素负载。该短门禁已越过不变的 99.5% 阈值并证明先前结构瓶颈关闭，但整项仍须用新实现重跑完整 30 分钟 recycle/recovery、冷启动/随机重启后才可勾选；多设备/驱动与声学 loopback 仍是独立验证矩阵，不能用假 Adapter、静态画面或事件注入替代。
