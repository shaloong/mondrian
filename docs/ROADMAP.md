...
本节是 2026-07-23 的代码基线，不是目标清单。状态区分“模型/算法存在”“已接入主路径”“已有真实项目证据”，避免从类型或单元测试推断产品完成度。
...
- [ ] CPU decoded cache、GPU/working cache、proxy index 都有字节预算、LRU/eviction、source revision 和颜色解释 key；整进程 Private Commit v1 已用 canonical 生成素材在 M0 固定参考机完成 Video+CPAL/A/V 基线。M1 仍须验证真实代理切换、多个素材/嵌套项目、8 GiB 最低档和更多 GPU/驱动的组合压力。
