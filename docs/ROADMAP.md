...
| 项目持久化 | `.mdp` manifest、schema v14 精确时间/跨媒体参数/Sequence author revision/统一时间显示设置/规范音频信号布局/Component 混音矩阵/可自动化 Route 边文档、当前 fixture、SQLite 素材库、临时文件写入、自动保存、恢复候选和重连已接入 | Alpha 明确拒绝旧 schema、不承诺兼容迁移；替换目标文件前的持久化/崩溃语义仍需压力验证 | L1- |
...
| 音频 | Track→Clip 已是 placement SSOT；Clip 持有 Component Edit，Sequence 持有 Processing Scope/Track Channel/Bus/Output/typed Route/Transition；Processor 参数持久化共享 Schema 快照与单一 exact curve，缺失插件可保留作者意图，内建 Gain 要求规范 Schema 精确匹配且不隐式补参数；播放与导出共用 `AudioProgramRuntime`、块式 decoder Adapter、嵌套公共输出、sample-accurate Gain/pan/fade/Transition、无隐式 clipping；semantic IR 已 prepare 为稠密拓扑 node slot、连续 Route/Contribution 区间、Transition binding、sample span、验证后的 automation event span 与 liveness scratch，执行时不扫描作者 Route/BTreeMap，也不逐样本校验或搜索作者曲线；Hold/Linear/Bezier 复用 core 的同一插值实现并保持 block partition exact；`AudioRenderSession` 在构造时冻结 `AudioRenderCapacity`，`render_into` 不扩容、不构造事件容器；静态 source map 以精确首样本 + `i128` 有理累加器覆盖分数正/反向速度；CPU scalar reference 与运行时 SIMD 共用 schedule，并有 1/8/32/64 轨 × 64/256/1024 frames 的 PCM 等价/p99 deadline 矩阵；产品 PCM 源使用文件指纹 + 原生语义布局 + 10 秒窗口 + 128 项/256 MiB 加权 LRU，Runtime 仅保留 4096 帧热窗；Prepared PDC 已求解并预分配 Contribution/Route delay，根级 stateful Runtime 具备显式 generation entry、失败整代作废、Synthetic Clock 恢复与有界 RenderBlocked；schema v14 的公共信号布局与 Component `Standard`/显式稀疏矩阵可表示 Mono、规范命名扬声器集合和 1–64 离散 Bus，明确区分 5.1(side)/5.1(back)/7.1；Prepared Contribution 在 Scope/增益/淡化/求和前统一执行 source→Sequence 矩阵，嵌套先按子 Sequence 布局输出再映射，播放/导出仅在根 Program Output 后做交付映射；素材库 schema v2 持久化稳定 Component/物理流签名/探测文件指纹，播放、波形和 Export 快照均验证 revision 后使用绝对 stream index；FFmpeg 只做原生序数 identity `pan`，不会把 Clip downmix 烧进缓存；Prepared Processor 保留实例/参数身份、Rack 顺序与 owner/insertion origin，共享 Scope 生成独立 Session state，并用预分配 sample-accurate Parameter-ID batch 驱动内建 Gain；主路由与并行 send 已统一为带 enabled/static gain/Sequence-time automation 的 typed Route，稠密 schedule 在 PDC 后按目标时间执行并提供 scalar/SIMD、分块一致性门禁；Processor 定义现由 semantic compile 原样保留、prepare 期 Resolver 绑定固定 latency/state/mode/scratch Factory、Session 构造独立实例，callback 只接收主 Bus/辅助输入查询和预分配 Parameter batch；普通 CI 用非零延迟 stateful Adapter 覆盖 PDC/entry/分块 PCM/mode 拒绝/失败 epoch poisoning；Program Output 已有 callback 无分配的逐通道 sample-peak/RMS/clip/non-finite block observation；本地真实 CPAL/A/V 30 分钟同步门禁已通过 | 真实隔离 VST3/CLAP Adapter、完整 mix-matrix 编辑器/预设、custom media layout probe、插件 bus/layout negotiation 与设备/编码 Adapter 的非标准布局协商、生产级非零延迟 processor、stateful nested reverse checkpoint/materialization、typed processor sidechain input、true-peak/响度/limiter/表头 ballistics 与 UI、完整 Rack/Automation 编辑 UI/undo 命令、固定参考设备矩阵与声学 loopback 仍未完成。GPU 只作为计入 PDC/批量延迟且能证明 deadline 的可选 processor backend，不能成为 callback 依赖 | L1+ 执行地基；不宣称 DAW 完成度 |
...
- [x] 建立 archive/document/SQLite version registry，并提供 schema v14 current document fixture、v0 SQLite fixture、幂等打开、事务回滚和失败不覆盖测试；Alpha 不保留旧 document schema 兼容。
...
- [x] Prepared latency solver 已在每个 Contribution→Track 与 port-specific Route→Bus/Output 汇合点计算补偿，累计 pre/post Rack 与 Program Output latency；嵌套 Runtime 自底向上传递实例级 child-output latency 与 state-entry obligation。Session 为每个 Contribution/Route 补偿输入预分配 fixed delay line 并报告容量；fresh continuity epoch、Offline 精确入口、Realtime generation Enter/Continue、失败中毒/有界恢复、独立 nested epoch、正向补算和 stateful reverse fail-closed 已进入产品路径与普通 CI。
- [x] Sequence、Render Contract、PCM request/cache/buffer 已统一 schema v14 规范信号布局；公共 DSP 可保真承载 Mono、命名扬声器集合和 1–64 Discrete Bus。素材库 schema v2 的 fingerprinted Component Catalog 已贯通播放/波形/Export 绝对 stream selection。媒体缓存保持 native-layout PCM；`Standard`/显式稀疏矩阵进入 Prepared Contribution，source→Sequence、child→parent 与 Program→delivery 是三个不混淆的边界，未知布局失败关闭。Prepared schedule 现保留全局唯一 Processor Instance ID、Rack 顺序和 owner/insertion origin；共享 Scope 为每个 Contribution 生成独立 Session state，Gain 通过预分配 Parameter-ID lane/sample-offset batch 执行，scalar/SIMD 与 block partition 保持 PCM 一致。主路由与并行 send 共用带 enabled/static gain/Sequence-time automation 的 typed Route 边；编译只将启用边纳入 signal closure，prepare 将 Route 曲线降为预分段 schedule，Session 只使用预分配 gain lane 并在 PDC 后按目标时间应用增益。
- [ ] 用户可见 Component 选择与显式重绑已贯通：Inspector 以稳定 Edit ID 在 Asset Component/嵌套公开输出域内选择并进入 Sequence Undo；资产级“重新探测候选→显式重绑”分离自动证据与用户意图，保留逻辑 ID，更新后会在当前 transport anchor 失效并重建音频 Runtime。Canonical authored mix matrix 的持久化、验证、稠密调度执行、嵌套布局边界和播放/导出交付分层已完成；仍须补齐完整矩阵编辑器/预设、custom media layout probe、plugin bus/layout negotiation 与非标准设备/编码协商。Processor Host 地基已把 semantic preservation、prepare-time Resolver/Factory、Session 实例、mode/latency/state/scratch、主 Bus/辅助输入查询和公共参数 batch 分层，并以非零延迟测试 Adapter 贯通 PDC；随后实现隔离的 VST3/CLAP Adapter、deadline/crash containment、state chunk 与真实 bus/layout 协商，并基于已执行的 typed Route/send 边实现 authorable processor sidechain、true-peak/标准响度和 Rack/Automation/Meter 编辑 UI/命令。首个非零 processor 必须同时交付真实 state、state-entry/latency/deadline 合同、PDC 与嵌套正向矩阵；GPU 仅可作为声明固定批量延迟、计入 PDC、设备丢失/状态切换可控且实测优于 CPU SIMD 的可选 backend。callback 实时安全、underrun 与 A/V drift 继续由真实门禁证明。
...
- schema v14 current fixture 可保存、重开；旧/未来 schema 明确拒绝；故意失败不会破坏源文件。
...
- 落地项目 version registry 与 schema v14 current fixture；Alpha 旧 schema 明确拒绝。
