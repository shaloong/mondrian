...
`GoldenProductWorkflowDriver` 持有一个真实 App composition、固定 `ProjectId + project path`；其私有 Hero binding 锁定初始 `SequenceId`、完整设置和 Project Color Environment。规范 v9 合同中的七个切片全部复用该 Hero 身份，只有 Recovery/Nesting 通过普通产品事务新增一个被 Hero 强引用的 Nested Composition child，因此完整 Project 精确包含两条 Sequence，而不是为每项能力建立诊断孤岛。组合门禁在 Visual 前后比较完整 Hero 音频投影，在 Editorial、Delivery、Proxy、Recovery 与 Color 前后比较各前序阶段的 Track-owned/局部作者锚点；Proxy 锚点覆盖重连后 Asset 与 proxy intent，Recovery 锚点只拥有自己的 parent Track 与 child，Color 锚点覆盖两条专属 Track、Clip、Asset 和共享 placement，允许后续阶段合法增量作者却拒绝旧内容漂移。此前八条隔离 Sequence 的三轮运行只保留为局部历史；新的正式监督运行 `20260725T172841Z-complete-golden-d631639d` 已在无人工修复和无清缓存步骤下，用三个不同 run/Project identity 连续 `3/3` 通过，三轮均报告七个 primary stage 共享一个 Hero、最终恰好两条 Sequence。该证据关闭 M1 Golden 的“单项目组合执行”门槛，但不替代独立绝对 HLG/PQ/Log reference、合格发布机、长时 CPAL/GPU/内存/A/V 同步或故障注入证据。
...
- [x] Golden Project 完整工作流由一个监督命令连续 3 次通过；正式运行 `20260725T172841Z-complete-golden-d631639d` 为 `3/3`，三轮使用不同 run/Project identity，七个 stage 的 primary `SequenceId` 均绑定各轮同一条 7,500 帧 Hero，最终均恰好保留 Hero 与一个 nested child，且没有人工修复项目文件或清缓存步骤。此前八条隔离 Sequence 的三轮组合结果只保留为局部回归历史。
