...
机器可读合同是封闭 schema v3：未知字段失败关闭；Sequence 固定 3840×2160、25/1、square pixel、progressive、linear Rec.2020 working、Rec.709 legal Program Output、10-bit 交付默认值和 48 kHz stereo；H.264/AAC 与 HEVC Main10 绑定稳定内建 preset ID 及完整 signal/probe 合同。五个可组合切片都只通过普通产品接口执行：Foundation Audio 证明 PCM 导入及 Clip gain/pan/fades、Undo/Redo 和耐久重开；Editorial/Transport 证明 AAC、Overwrite、Split、Ripple、跨两条显式 ripple Track 的专业 Insert、Scrub、Accurate Seek 和 Synthetic Clock Master Play；Proxy/Relink 证明 canonical Proxy→Original→Proxy、结构化离线、同一 `AssetId`/Clip 重连及旧代理失效；Generated Delivery 证明 Transform/Opacity、H.264 High 与 HEVC Main10 真实导出、严格 probe 和普通媒体重导入；Visual Authoring 证明 Cross Dissolve、Basic Title、Hold/Linear/Bezier，以及工作空间感知 Primary Color 和显式 Rec.709 域 LUT 的产品作者事务。Visual schema v7 还固定 Effect ID/顺序、Primary 参数、LUT 路径与完整内容哈希、`DOMAIN_MIN/MAX`、四面体插值声明和 Preview/Export 编译图签名，耐久重开后必须逐项相同。五个切片已在本地真实通过，但生成画面与 identity LUT 只提供回归一致性证据，不是独立色彩 reference；短窗播放也不替代 30 分钟 CPAL/GPU/内存/A/V 门禁。各报告只能关闭自身声明的局部义务，HLG/sRGB Alpha 合格参考、真实媒体/嵌套色彩、恢复和连续三轮顶层 Golden 仍未完成。

顶层 `GoldenAcceptancePlan` 把 fixture/operation/content/export 全部义务编译成确定性账本，并明确报告未规划与未绑定项；计划与五个切片报告都固定 `complete_golden_project: false`，只有单一 Project 身份的总运行协调器能在完整观察后声明通过。专业 Insert、Primary Color 与 LUT 接入后，operation、content 和 export 账本已经没有未规划项；当前计划阻塞只剩未绑定的 HLG Main10 与 sRGB Alpha 图片角色及其合格 fixture。专业 Insert 仍使用独立 target/ripple scope、链接组、锁定、Transition 和 Sequence-time automation 合同；`ClipOverlapMode::PushForward` 不得被重新解释为 Insert。Rust 与 PowerShell 校验器均按 schema v3、三轮要求和 slice export 引用失败关闭，不能由旧脚本或进程退出码虚构完整性。

单 Project 执行地基现已开始收敛：`GoldenProductWorkflowDriver` 持有一个真实 App composition、固定 `ProjectId + project path` 与该运行的全部阶段 Sequence；Project-scoped 作者事务只要求 Generation 精确前进并允许活动 Sequence 改变，Sequence-scoped 事务还要求活动 Sequence 不变且其 Revision 精确前进。普通 CI 已证明同一项目中新建第二阶段 Sequence、耐久保存、关闭和重开后 Project/path/两个 Sequence 均不漂移且只有 Session 身份更新。Foundation Audio、Visual Authoring、Editorial/Transport（含多轨 Insert）、Proxy/Relink 与 Generated Delivery 五个真实切片均已成为可组合阶段：同一 Project 依次保留 PCM 作者内容，Transition/Basic Title/Primary/LUT 效果栈，AAC 编辑与 Transport 状态，同一 Asset/Clip 的 replacement 路径与代理意图，以及 H.264 High/HEVC Main10 导出与重导入；最终 durable reopen 对照五份完整 Sequence 快照、Project/path、效果身份/参数、重连资产和两个成品 profile。顶层 Coordinator、独立真实色彩/嵌套/恢复义务与连续三轮完整运行仍明确阻塞。
...
- [ ] 预览/导出使用同一 timeline/effect/color/alpha 解释；Golden frame 与 report signature 对齐。视觉作者切片已对同一帧证明 Basic Title 语义、Cross Dissolve progress、Primary+显式域 LUT 编译图签名、float-linear CPU path、raster signature 与像素哈希在 Preview/Export 及保存重开间不漂移；真实媒体颜色/Alpha、代理、嵌套、GPU LUT 与编码后独立 reference 仍未全部纳入同一顶层报告。
