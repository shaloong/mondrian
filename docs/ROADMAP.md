...
- 已让 CPU-F32 scalar ROI executor 消费同一 demand：精确输入 tile 保留完整画布坐标，有限核消费实现推导 halo，full-frame/unknown 证据不降级；编译 schedule/use-count 直接驱动 last-use move、fan-out clone 与 Blend/MultiInput join 释放，resident value、provider transient、output crop/Arc publication 和 kernel scratch 纳入一个 Session byte grant，边/字节账本不闭合则失败。Blur/Vignette/Grain 以及有限历史→current-time DAG（含坐标种子 Dissolve）已有独立 current/full-frame reference crop parity。下一步是不改变该语义参考地增加完整画面的调度级切片、Mask 的 coordinate-aware cancellable raster、temporal×heterogeneous 与 CPU/GPU parity；不得用裁剪后的错误像素换取性能，也不得把 executor 的局部能力写成 Preview/Export 已自动分块。
