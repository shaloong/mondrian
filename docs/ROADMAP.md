...
3. **跨平台生产实现必须完整，实机资格分阶段取得。** Windows、Linux、macOS 必须共用项目、时间线、播放、音频、效果、色彩、Viewer 与导出契约；平台差异只位于 `mondrian-platform*`、媒体/渲染器原生后端或明确 Adapter 内。M1/M2 允许只以 Windows 真实 GPU、硬解、音频设备和显示器证据阻止发布，但这不能授权共享 Module 依赖 Win32、D3D12 或 Windows 行为，也不能把 Linux/macOS 的缺失实现伪装成“已预留”。实现存在、原生 CI 编译、真实设备正确性和发布打包是四类独立证据。
...
| 项目持久化 | 当前格式为 document v25/library v5。`AuthoringSession` 生成不可变作者与 SQLite revision 快照；SQLite online backup、身份绑定 sibling archive 与唯一 Storage Seam 共同产生 durable publication evidence。只有 durable evidence 可推进保存 baseline 或清理 Recovery Authority；旧 Session、旧 Persistence Generation、过期 destination 和非耐久结果都无权发布。运行时 Library 采用不可变代际目录与 path-paired lease，已打开目录不被 rename/swap；Recovery 以规范 manifest、文件身份和完整 Project/author/library/document evidence 闭合发现、选择、加载、保留与退役。Window close/quit 已使用 move-only 非阻塞 pause ticket：Save 与 FIFO barrier 保持一个 worker 顺序且绑定精确请求，作者 Action 和 Project-scoped completion 在 quiescing 期间冻结，UI 以有界 cadence 保持响应；保存失败会恢复 generation，协议故障只允许显式 Discard 后无权威 detach。有界请求队列现先准入 payload、再取得目标路径权威；队列拒绝不会改变 destination/revision 或残留路径锁，饱和队列→Save As 拒绝→精确保存后关闭已有组合门禁 | Alpha 对非 current document 明确拒绝且不承诺迁移。M1 仍须以恢复来源/时间/目标/冲突 UI 为输入，验证真实磁盘满、权限失败、反复崩溃、close/Save As 和慢盘下不覆盖源、不误清 Recovery Authority、旧 completion 不发布，并提交三轮可重现故障证据 | L1 所有权、代际、正常恢复、队列权威与非阻塞 close handoff 闭合；M1 实机文件系统/交互证据未完成 |
| Undo/Redo | `AuthoringSession` 是唯一可变 `ProjectDocument` 权威。Sequence 与作用域化 Project restore point 是两种连续 typed-state endpoint；Undo/Redo 在移动栈前验证 source/scope、复用 unaffected Sequence、深验完整 Project，并保留当前 document revision、更新时间与有效导航。no-op 在 mutable access 前返回，只有 touched Track/collection detach；History 以 200 条/256 MiB 的版本化 logical charge 计费，未保留事务建立连续性屏障，且该计费不得冒充 allocator/RSS | 当前机器可读门禁必须在同一优化构建和 source attestation 下完成 5/30/120 分钟 × active-heavy/project-heavy 六格，输出连续 6 reports + completion；同时证明 endpoint 连续性、scope/source mismatch 失败关闭、COW locality、200 条深度、120 分钟 precompose Undo/Redo、正常路径无 oversize/barrier，并把专用 retention-disabled/oversize/barrier probe 与正常路径分开。Private Commit 双门禁和 History logical charge 必须独立判定。本轮 source `sha256:c817441543da43e6c1b8f292bd174d6618922b2ce792772967ee5825aa29a12e` 的 schema 10/protocol-v1 正式运行已产生连续 6 reports + completion，reference v4、Locality v3、History 与 Windows Private Commit 全部通过；120 分钟 project-heavy 的 Project JSON 为 196.76 MiB，最慢发布 p95 6.420 s/20 s，200 条 History 为 173.35 MiB/256 MiB，Private Commit 峰值 540,827,648 bytes、增量 438,398,976 bytes | L1 事务架构与本轮定量矩阵闭合；8 GiB cross-load、产品级交互和最终候选 source 重跑仍未完成 |
...
| 时间线编辑 | 多轨 Move/Split/Trim/Ripple/Overwrite/Roll/Slip/Slide/Insert/Lift/Extract、锁定、吸附、多选、Link Group 和嵌套已有类型化作者事务。Insert/Lift/Extract 显式携带 content/target/ripple Track、半开范围及 automation/Transition/navigation 策略；候选求值与执行共用验证，锁轨、链接闭包或 handle 不满足时整项失败。Track Targeting 与 Sync-Lock 是独立会话状态；定向 Split 返回完整链接成员身份 receipt。Transition 使用强端点、精确范围和 unclamped 双源 handle demand，创建/缩短/删除各为一次 Undo | Golden v14 已在同一 Hero 中通过当前声明的 Lift/Extract、Link、Transition、嵌套、Undo/Redo 与保存重开切片；仍须以复杂 J/L linked edge、mute/visibility、真实媒体与嵌套 Transition、reverse/freeze/variable remap 为输入，定义 source/handle/automation 传播和失败提示，并完成 T/S 工作区持久化、代理/原片与独立视觉 reference | L1+ 作者、命令与基础 UI 闭合；扩展产品交互和媒体边界证据未完成 |
| 播放与缓存 | UI 无关 `PlaybackEngine`、`FrameWorkBroker`、`PreviewFrameStore`、三种访问语义、FFmpeg Session/isolated helper、generation/cancellation/deadline 和 Window/Headless production runtime 已接入。解码选择以 `DecodedTemporalExtent` 的半开 `[start,end)` 为权威：正 duration 与首个 successor 共同收紧边界，跨 forward call 最多保留 selected + successor 两个候选。Playback/Still 只接受因果覆盖请求的帧，否则失败关闭；Scrub 才可把非覆盖近邻标为 Explicitly degraded。取消记录 request→checkpoint→return，旧 generation 不得发布。单一 `viewer_gpu_device_progress` 非 UI worker 驱动 exact submission/callback；current/prepared 物理 publication 由唯一 UI 无关所有者管理，Window/Headless 只附加 artifact。提交 `0175f35` 的 4K Video 单门禁已在 16 GiB Windows 参考机自然完成 30 分钟/45,002 帧、50 次 warm seek、52 次 accurate seek、100 次 supersession、exact GPU completion、取消、helper、进程树内存与 post-stress 收敛；提交 `78a4d99` 又通过完整 30 分钟真实 CPAL recycle + Headless GPU 门禁 | 两项长门禁来自不同 clean revision，因此最终候选仍须在同一 source attestation 下重建 Video+Audio gate set，不能把历史报告拼成一份 baseline。还须补 Window replacement/handoff、多 GPU/驱动/音频设备、真实阻塞媒体、8 GiB 降级和声学 loopback；单个平台/设备通过不能冒充这些矩阵 | L1 代码契约、Windows 长时视频与真实 CPAL 单门禁分别闭合；同 revision baseline、设备和 Window handoff 资格未完成 |
| 执行资源治理 | 版本化协调 Module 把物理内存档位、七个重任务域和进程/系统压力投影为不可变 admission decision；各域仍独立拥有队列、deadline、取消和失败分类。Windows、Linux、macOS 已分别以系统 API、`/proc`、Mach/libproc 实现物理/系统内存和完整产品进程树观测；结果携带不可互换的 Windows Private Commit、Linux anonymous resident 或 macOS physical footprint metric。平台实现按 `memory`、`process_memory`、`playback_scheduling`、`display/{windows,macos,linux}`、`global_pointer` 与 `eyedropper` 深 Module 保持 Locality，`lib.rs` 只负责组合；Windows MMCSS、macOS pthread QoS 与 Linux per-thread nice 均为线程仿射且可恢复，Linux UI/event loop 不进入实时调度类。原生优先级不可用或失败时，本次驻留稳定进入 `PortableFallback`，Pause/Stop 后才重置并允许重试，不产生 `Unsupported → Inactive` 的伪状态跳变。8/16/32 GiB 分别开放 1/2/4 个粗粒度域槽，Preview/Export/Viewer 在执行前冻结 hard grant | 必须在独立 8 GiB 环境以 Preview+音频+Import+Thumbnail+Waveform+Proxy+Export 为输入，证明实时音频/UI 不饥饿、no-overlap handoff、旧结果不发布、压力恢复可重复、内存/吞吐有实测证据；平台资格报告必须绑定 exact metric，Windows Private Commit 门禁不得接受 Linux/macOS footprint 替代。逻辑 byte grant 不得冒充 VRAM/RSS | L1 调度、工作集策略、平台代码 Locality 与三平台观测 Implementation 闭合；跨负载实机门禁未完成 |
| 原生硬解/低拷贝 | Windows D3D11/D3D12、macOS VideoToolbox/CVPixelBuffer→Metal、Linux VA-API→DRM PRIME DMA-BUF→Vulkan 已接入同一生产 Interface。媒体层分别保留并验证原生资源；Metal/Vulkan 薄 Adapter 只校验/包装平面，YUV sampling、range/matrix/chroma、source-to-working OCIO 与 working-frame 资源事务由同一 Renderer Module 执行。Linux 仅接受 FourCC 与 NV12/P010 一致、单 typed layer、双平面、object/offset/pitch/modifier 全部可证的描述符，其他布局结构化回退；报告区分 native GPU、upload、native bridge copy、readback、degraded 与 blocked。平台层的平行 GPU probe 已删除：只有当前 Renderer Adapter/Device/Queue 可发布不可变 import support 及显式 `ZeroCopy | GpuBridgeCopy` 模式，App 再与实际媒体 surface facts 组合准入。Metal/Vulkan 直接外部纹理路径可声明 zero-copy；当前 D3D12 共享纹理桥必须声明一次 GPU bridge copy 并报告 low-copy，绝不冒充 zero-copy。当前提交的 Windows 4K25 HEVC Main10 长时门禁观察到 45,003/45,003 个原生 P010 硬件层、100% hardware execution、45,105/45,105 个 GPU import timing sample，以及零 CPU transfer、readback、fallback、GPU blocker、缺失、丢弃或重复 sample | Windows 当前参考机单视频门禁已通过，但完整 Video+Audio baseline 仍未形成。macOS 平台 Adapter 已通过 `aarch64-apple-darwin` 交叉编译；Linux 从 Windows 交叉编译在本仓库代码前被 DBus/pkg-config sysroot 阻止。两者仍须由原生 CI 编译完整 App，并以真实 Metal/VideoToolbox、Vulkan/VA-API 设备做像素 reference、同步、seek/cancel、device loss、内存与性能资格；交叉 target 不能冒充平台验证 | 三平台生产路径、单一设备能力权威与显式复制模式已实现；当前 Windows 长时原生 Main10 视频资格通过，Linux/macOS 原生 CI 与实机资格未确认 |
| 渲染与色彩 | Preview/Export 共用按 Sequence/Registry revision 与作者 fingerprint 绑定的不可变 `PreparedVisualProgram`；逐帧 `PreparedVisualFrameClosure` 固定 nested time、canvas/color、Transition/temporal binding 与 instance path，区间 `PreparedVisualRangeClosure` 固定 selected-range 的媒体、Transition 和字体依赖。`CompiledEffectGraph` 是唯一生产 IR，动态 topology/cache 只驻留于调用方 `EffectExecutionSession`。生产帧入口由 `TimelineCompositeScratch` 一次绑定 Generation，并用同一 Session 完成普通 Render Plan 求值、全部 temporal root/sample 精确时刻重求值及后续像素执行；Preview/Export 不再拼接无 Session 的第二套 temporal 准备。有限时域 CPU-F32 执行把根图、精确请求、按 `(Clip time, graph value)` 寻址的 time-expanded program 与原始 Source demand 冻结为一个对象；temporal tap 可采样稳定 Definition-stage input，并在各采样时刻从同一 Prepared Effect Program 重求值全部上游效果、动画参数、动态 topology、frame seed 与 Mask。expanded schedule/use-count 驱动精确去重和 live-set；完整请求超出 grant 时会在最多 4,096 个非重叠 tile 内自动执行。`PreparedMaskRaster` 按每个可达时间上下文准备 Path/BVH，以全局坐标、受限 row scratch 和合作取消服务 direct/tiled 路径；Blur/Vignette/Grain、Mask、含 Dissolve 的 temporal DAG、signed multi-tap、sampled upstream 参数差异、同代 topology 复用及换代清空均有 evidence。异构 CPU prefix 已按唯一 value plan 执行 source-closed DAG、synthetic MaskSource 与 fan-out/join live-set；CPU frontier 可原子发布多个精确 boundary materialization，并为每项保留 non-converting `EffectValueFormat` 后分别 upload，GPU suffix 仍直接消费同一 plan 的 dispatch/release schedule。Effects 现持有版本化的 opaque route-shape identity（完整 dispatch/transfer/release、lane/backend、value format 与 operation kind；排除 extent/参数），Renderer 只暴露该身份与全部 frontier 的精确 retained bytes；Export route ledger 不再遍历节点重建 fingerprint 或只接受 RGB，Preview batch 也按每个 input 加全部 frontier 精确准入。Preview 的 closure-level payload finalization 现让父级 Nested Clip 使用已完成 CPU 物化的 child working frame，并以 child 的精确 execution raster 准备异构 route；child 自身仍要求完整 CPU 路线。线性 point tail 保持 fused pass，共享 GPU value 可经二元 Blend 或 ordered MultiInput 汇合任意 canonical scene-linear BlendMode。多输入折叠的私有纹理与单输入的位精确 texture copy 都进入同一物理 recording bytes/texture-count authority。CPU MaskSource 上传为 `NonColorData/AlphaMask` 而非伪 Working 色帧；GPU Mask 单 pass 实现四种 MaskOp 与 inversion，只改 straight alpha。共享 compositor 保留 authored opacity 与完整 64 位 Dissolve frame seed，全部 BlendMode、两条独立 CPU spatial 分支→GPU join、CPU MaskSource→GPU Mask 八组组合、三输入 SoftLight 和单输入 copy 已用真实 wgpu 完成 CPU Float32 parity，其中 copy 要求逐位一致。wgpu Adapter 在记录前校验所有 transfer format/materialization/wait/signal/release/terminal live-set，并在任一后续 upload 失败时清理此前未发布资源；单 command buffer 无物理别名时的保守物理要求与抽象 plan peak 分开准入。Mask preparation/raster 保留 attempt 的取消/deadline 原因，提交开始后的取消、GPU 或证据失败是本次 attempt 终态，不隐式整图 CPU 重跑 | Temporal 与异构仍是 bounded execution：internal same-stage temporal address、unbounded/stateful、GPU 后再切换 backend 或图内 readback、除 AlphaMask 非转换传输外的颜色域转换、temporal×heterogeneous 和这些组合形状的广义 GPU parity 未闭合。后续必须以生产有限时域/扩大 ROI Processor、更多嵌套组合和 placement 为输入，证明 exact window、halo/full-frame/unknown、continuity、resource lifetime、seek/cancel 与 CPU/GPU reference parity；缺少 exact mode、live-set 或 Adapter 时在像素执行前阻止。Log/HDR 仍需独立 reference | L1+ 唯一 IR、Prepared closure、精确 ROI、effected temporal upstream、owner-scoped temporal topology、受限自动切片、bounded temporal DAG scalar 与 bounded GPU multi-frontier/fan-out/Blend/MultiInput/Mask 边界闭合；通用 Temporal/ROI/异构执行未完成 |
| 效果与动画 | 稳定 Effect/Parameter/Keyframe 身份、版本化 Parameter Schema、Property/Animation、DAG、Mask 和 plugin definition/DSL 已接入。Clip 直接启用状态、Solid Color、Transform/Opacity/Basic Title 参数及数值曲线由唯一 `ClipProductAction` 承载；Inspector、Viewer、Headless 只投影同一 Interface。参数批次只携带 `ClipId + AnimationParameterAddress + PropertyValue`，当前 Track/锁定与 Clip-local author time 在深层 Module 重解析；任一空批次、重复、stale、类型错误或无效成员会在候选前整批拒绝。曲线权威时写入完整 key，保留 Keyframe 身份、Bezier handle/插值/flags，不在曲线下制造隐藏 static 值。Effect 子实体仍由独立 `VisualEffectProductAction` 承载：添加只实例化当前可执行注册 Definition 并为每个实例 fork owner-local `AnimationTrackId`；排序使用 EffectId 相对 anchor；参数写使用稳定地址，字符串 path/index 不充当外部身份。Mask 子实体由独立 `VisualMaskProductAction` 承载：Track/Clip-local time 在事务前重解析，Track/Mask 双层锁、稳定相对排序、完整形状写入与 scalar 参数地址共用唯一 Interface；每个 shape key 具有稳定身份和显式 Hold/Linear，Linear 只接受兼容拓扑，静态 Mask 规范为单一 time-zero Hold key。Basic Title 与 Cross Dissolve 已贯通作者、Undo、保存重开和共享 Preview/Export 路径；效果库只展示可构图定义 | 仍须以 Ease/handle、copy/paste/reset、长文本/缺字、真实媒体/嵌套 Transition 及首批算子为输入，逐项证明产品可选、图可执行、CPU/GPU backend、缓存失效、Preview/Export parity 和独立视觉 reference；没有 render op 或 backend 的 definition 必须保持 blocked | L1 地基、Clip 直接参数与视觉 Effect 作者 Action 闭合；首批算子与产品广度未完成 |
| 音频 | 当前 document v25 承载规范 signal layout、Component matrix、typed Route/send、Rack、Automation 与 Transition；Track→Clip 是 placement SSOT。播放、导出、Audition、Analysis 和嵌套共用 `compile → prepare → Session → Runtime`；dense schedule、PDC、tail、lookahead 和 Processor Host 均有显式预算，callback 不扫描作者图、不分配、不做 I/O 或隐式 clipping。Clip/Scope/Channel/Route/Processor 数值自动化共用稳定 `AudioAutomationTarget`、显式 `AuthoringTimeDomain`、单一事务 Interface 与同一 CurveEditor Adapter；根 Runtime 接受 open-Session solo overlay，嵌套输出保持 canonical，Track/Bus/Program Output post-mute meter 以 block-atomic bank 无锁发布。实时输出以稳定 CPAL `DeviceId` 区分 `SystemDefault` 与精确设备意图，设备变化经 typed Lost→Synthetic Clock→新 stream generation 重协商，缺失的指定设备不静默回落。提交 `78a4d99` 已通过 `cpal_av_48khz_30min_recovery_v2`：两代真实 stream、一次精确 controlled recycle、最终 generation 1,801.153 秒 callback、Ready 99.72%、Audio/Synthetic proven phase 最大 13.299/1.554 ms、零 underrun/替代/recovery，缓存与完整进程树内存合同全部通过 | 仍须以多布局设备、stateful reverse、sidechain、独立 oversampled true-peak/响度、更多生产 Processor、外部插件 Adapter 与完整自动化/solo/meter Golden 为输入，证明 layout negotiation、PDC/entry/tail、实时 deadline、嵌套与播放/导出 PCM 一致。当前 meter 与 limiter 只承诺 sample peak，不能作为 true-peak/响度证据；CPAL 也不能证明 named multichannel speaker 位置/顺序。多设备/驱动、默认/指定设备切换矩阵、冷启动/随机重启、声学 loopback、8 GiB 和非 Windows 实机仍未关闭 | L1+ 执行地基、Mixer/Rack、基础精确自动化、瞬态 solo、分层 sample meter、跨平台设备选择产品路径与当前 Windows 长时 CPAL 门禁闭合；插件、标准响度、多声道位置及发布设备矩阵未完成 |
...
6. **音频/视频运行许可必须持续分离。** `Priming` 可预填 PCM 但禁止设备提前消费，普通视频 Late/Recovering 不旋转音频 generation。当前 Windows 参考机已有正式长时 A/V 门禁证据，但仍须在多设备/驱动矩阵覆盖慢首帧/seek、持续视频压力、设备失效和声学 loopback；单机证据不得外推为全平台发布级同步保证。
...
Platform OS Adapter Contract
        ├── Windows（shell/display/memory/scheduling/global pointer）
        ├── macOS（shell/display/memory/scheduling/global pointer）
        └── Linux（shell/display/memory/scheduling/global pointer）

Device-bound Native Media/Renderer Contract
        ├── D3D12VA surface ↔ active DX12 Adapter/Device
        ├── CVPixelBuffer ↔ active Metal Adapter/Device
        └── DRM PRIME DMA-BUF ↔ active Vulkan Adapter/Device
```
...
- Windows、macOS、Linux 与 Noop/headless 都必须实现同一小型 Platform OS Interface，不复制业务逻辑。剪贴板、文件对话框、文件定位、显示、内存、线程调度和全局指针等 OS 能力属于平台 Adapter；D3D12、Vulkan、Metal、VideoToolbox、VA-API 等资源与同步原语只能留在媒体/渲染原生 Adapter。共享 Renderer/Media Interface 只携带类型化能力、资源身份、同步义务和执行证据。
- 原生视频导入能力只能由拥有当前 wgpu Adapter/Device/Queue 的 Renderer Runtime 证明；媒体层证明实际解码表面与 sampling facts，App 只组合二者。禁止平台层按 OS 名称宣称支持，也禁止另建一个图形设备做 capability probe，因为它不能证明与当前 Renderer 的物理设备、feature、allocation 和 synchronization 一致。
...
- M1/M2 的跨平台工程门禁至少包括三平台产品入口编译、平台 crate 的三平台 Clippy、共享 CPU reference、平台能力合同、无平台类型泄漏和 Adapter failure/degrade 测试；Windows 另承担当前真实 GPU、硬解、音频、显示和长时运行资格。缺少 Linux/macOS 实机证据必须明确记录为资格缺口，不能反向降低其生产实现要求。
...
当前机器可读合同是 `windows-alpha-golden-v14`、封闭 schema v4；未知字段、未执行义务和缺失执行前提均失败关闭。Hero Sequence 固定为 7,500 帧、3840×2160、25 fps、square pixel、progressive、linear Rec.2020 working、Rec.709 legal Program Output、10-bit 交付默认值与 48 kHz stereo；H.264/AAC 和 HEVC Main10 使用稳定内建 preset ID 与完整 signal/probe 合同。
...
- [x] 帧工作统一 generation、priority、deadline、取消 disposition、请求龄期、资源预算与 terminal evidence；deadline 只在 Adapter 边界降低一次，同键 rebind 不续期，worker 发布前记录完成时刻，时钟回退会钳制并令专业门禁失败。Playback next-wake 保留 Priming/Presentation/FrameBoundary/AudioDevicePoll 类型化原因；终态 ticket 永不续期，也不得由 timer 或后续 Clock observation 复活。Waveform、Thumbnail、Proxy 与 Export 分别拥有有界队列、缓存/重试、generation cancellation 和终态历史；它们只共享值语义，不共享万能 Job、worker pool 或容量。
...
- [x] 已建立版本化 Reference Corpus、Golden/Stress 机器合同、Windows 参考机 profile、机器资格和分层门禁。generated fixture 以固定 recipe 加 run-local artifact attestation 识别，不能伪造跨 encoder 位级稳定性或给陈旧产物重新背书。Playback 合同固定 Video+Audio 输入用途、45 分钟监督期限、进度 journal、16 GiB baseline profile 与结构化结果；Golden 使用 `windows-alpha-golden-v14`/schema v4，固定 Hero、七切片 role/export/window obligation 和“未执行不得通过”。
- [x] Golden v14 由单一深 Module 编译全局义务与 Hero 绑定两份账本，并校验 schema v4、Sequence role、Hero 身份、三轮规则和 slice/export 引用。Planner 即使结构完整也必须保持 `complete_golden_project: false`；只有监督执行观察到全部义务才可完成。删除义务、把 role 改为 diagnostic、单切片成功或隔离 Sequence 并集都必须重新产生精确 blocker。
...
- schema v25 current fixture 可保存、重开；Project 色彩环境/新 Sequence 模板、封闭 Sequence `color`/`delivery`、Clip-local 视觉作者时间、单一 Clip 源时间映射、Basic Title/Mask 动画与强编辑关系 round-trip；旧/未来 schema 与未知作者字段明确拒绝；故意失败不会破坏源文件。
...
- [ ] 以 H.264、HEVC、AAC、PCM/WAV、PNG/JPEG 及损坏/截断/不支持变体为输入完成有界后台导入；probe、fingerprint、stream/layout/color 证据必须原子提交，旧 generation 不发布。不支持或损坏输入返回可操作原因且 UI 仍响应。当前产品可执行文件已提供三平台共用的版本化 Isolated Media Probe Helper：按 canonicalize→完整 fingerprint→FFmpeg probe→同 revision fingerprint 顺序准备候选，父进程以 8 MiB stdout 上限、64 KiB stderr tail 和 120 秒单调 deadline 监督，取消/超时须 kill 且 reap 后才发布终态；Import、Relink 与 Audio Component refresh/rebind 共用该物理 Adapter，但保持独立队列、generation、提交门和证据。真实小 H.264 素材已覆盖完整产品入口协议，睡眠 Helper 已覆盖取消/deadline 回收。仍须完成全部格式/损坏 corpus、批量交互、项目切换与保存重开组合、同路径替换和 8 GiB 压力验收，故本项不提前勾选。
...
- [ ] durable worker、SQLite snapshot、Session/generation completion、dirty/manual/autosave baseline 和深 Recovery Module 已完成；fixture-free Golden 已在 Hero Sequence 的独立 `175..200` 窗口覆盖正式 Precompose、Autosave、关闭原 Session、产品恢复 Action、恢复点保留、当前手动保存退役和再次重开。Hero primary、强引用 child、完整 parent/child 作者快照、Preview 像素与 Export 递归执行均不漂移，且组合协调器会复核此前阶段锚点。Window Save/Discard→Close/Quit 已由非阻塞 pause ticket 闭合，受控慢保存证明开始调用立即返回、后续作者 Action 冻结、精确保存请求耐久后才退役 Session；保存发布失败会保持 Project 并恢复 admission，协议故障须显式 Discard 才退役且迟到 completion 无权发布。持久化完成现在以 typed failure category 表达磁盘耗尽、权限拒绝、目标冲突/消失、非法数据、其他 I/O、内部错误和请求拒绝，并与 `BeforeNamespace / DurabilityUnconfirmed / NamespaceIndeterminate` 正交；确定性 worker I/O 注入已证明磁盘满/权限失败不覆盖 canonical bytes，磁盘满的手动保存也保持 Project 打开且 Recovery Manifest 字节不变。启动恢复现为显式检查→确认两步：Modal 展示精确 autosave 来源、绝对/相对时间、预期保存目标及 typed canonical target presence/revision，说明恢复只打开 dirty Session、不立即覆盖文件；目标在确认期间被创建、删除或改变 revision 时 preflight/lease 内复核会安全拒绝，Window Adapter 会刷新候选并保留可操作失败原因。三轮独立子进程会依次恢复上一轮、提交新编辑、耐久 Autosave 后直接 `abort`；父进程已证明 OS 锁释放、第三轮 dirty 作者状态可恢复，且最终覆盖保存退役全部 Recovery Authority。仍须补齐真实文件系统权限/容量故障矩阵、更细分的冲突恢复指导和 Save As/close 实机压力边界，因此本项不提前勾选。
...
- [ ] 三平台生产路径已经能够把 D3D11/D3D12、VideoToolbox/CVPixelBuffer、VA-API/DRM PRIME 输入同一 GPU YUV→working 执行链；下一门槛以 4K23.976–60 HEVC Main10 Long-GOP 为输入，在支持设备上观察逐帧 hardware→native surface→GPU YUV/working 证据。Windows 完成 M1 发布资格；macOS/Linux 先通过原生 CI，实机资格可晚于 Windows。缺设备、格式、可信 DRM layout、同步或预算时必须自动选择 CPU transfer、低分辨率或代理并显示原因，不能错误读回、改色或伪报 zero-copy。当前 `0175f35` 的 4K25 视频门禁已在合格 16 GiB Windows 参考机以 30 分钟 cadence 通过：45,003 个媒体层全部为原生 P010 hardware execution，GPU import 45,105 个 sample 全部闭合，CPU transfer/readback/fallback/blocker 均为零；进程树 Windows Private Commit 峰值 1,215,262,720 bytes，settled growth 17,589,808 bytes，post-stress 为 942,784,512 bytes。该单 gate 仍不代表 23.976/50/60、多 GPU/驱动或完整 A/V baseline，因此本项不勾选。
  2026-08-06 又以本地临时 4K59.94 HEVC Main10 BT.2020/PQ 长流在当前生产实现完成修复后的 clean revision 正式门禁：1,800.030 秒内 107,895/107,895 Ready、零 Stale/Unavailable/MissedDeadline；107,896 次 GPU 执行全部完成并精确分类为 2 次 `PublishedCurrent` 与 107,894 次 `PreparedSuccessor`，另以 107,896 次 unique presentation 独立证明后继准备未冒充呈现，零 Released/TerminalRejected/fallback。全部 107,896 个媒体层为原生 P010 hardware，renderer import timing 107,998/107,998 且零 pending/missing/drop/mismatch；50 次 warm seek p95 17.196 ms、52 次 accurate seek p95 113.819 ms、100 次 supersession，结束时 Broker pending/queued/in-flight/clock-regression 与 rejected terminal delivery 全为零。完整 1–3 进程树共 1,802 次 Windows Private Commit 采样全部完整，峰值 1,119,883,264 bytes、稳态增长 60,126,139 bytes、post-stress 1,035,288,576 bytes，取消、专业媒体和外层真实媒体合同全部通过。该结果关闭本参考机的 59.94 clean-run 缺口，但不代表 23.976/50、多 GPU/驱动、8 GiB、完整 A/V baseline 或 macOS/Linux 实机资格，因此本项仍不提前勾选。
  当前 source 已把专业视频 profile 升为 `uhd_hevc_main10_hardware_1x_v7`：每个有界 Headless 候选回合必须先推进完整资源周期，GPU completion、Prepared Successor 和 exact-current alias 快路径均不能跳过进程树内存 cadence 或 Preview/Viewer 资源投影；机器可读门禁要求应用次数至少覆盖计划观察帧数。上述 v6 结果仍是历史设备证据，但不满足当前 v7 source attestation，最终候选必须重跑，不能沿用旧报告宣称当前通过。
- [ ] seek/scrub 必须 latest-wins：新意图原子取代旧 generation，旧请求在固定 checkpoint/return 预算内结束且永不发布；settled seek 仍可重新提交 exact Still。当前正式视频 gate 已证明 50 次 warm seek（p95 15,392 us）、52 次 accurate seek（p95 356,722 us）、100 次 supersession、零 rejected terminal delivery，且结束时 pending/queued/in-flight/clock-regression 全为零。仍须补连续拖动产品交互、暂停/项目切换、Window replacement 与迟到 callback 的独立实机故障矩阵，所以本项保持未完成。
...
- [ ] 完成 custom media layout probe、Sequence→设备布局协商和非标准设备/编码拒绝的产品证据。当前 library v5 已让 FFmpeg native speaker mask 精确进入 canonical layout，未知顺序/位置保持 Unsupported；Playback/Export 已共用 `AudioProgramDeliveryRuntime`，其 evidence 含 Program/target layout、Identity/Standard/ProvenSilence 和 coefficient count；Export preset admission、FFmpeg 命令与完成 probe 共用一份编码布局能力表，MP3 非 Mono/Stereo 及无显式 lowering 的 custom/Discrete 在入队前拒绝。实时输出逐次枚举所选设备的真实 `supported_output_configs`，只选择精确 rate/channel 候选并确定性选择全部已知 scalar format；成功 evidence 保留 host、稳定 DeviceId、选择意图、default 事实、name-query、semantic layout、buffer range 与候选收敛计数，失败保留稳定拒绝码/request/selected contract/backend detail。`SystemDefault` 与指定 DeviceId 已贯通后台发现、Preferences 持久化、运行时切换、默认设备变化检测和新 generation 重协商；缺失指定设备保留意图并失败关闭，不按名称/index 猜测，也不在 UI/callback 枚举。CPAL 仅给通道数，因此目前只承认版本化 Mono/Stereo 约定与 ordinal Discrete；named 5.1/7.1/custom speaker 即使数量相同也拒绝。仍须按平台证明 speaker 位置/顺序的多声道 Adapter，并以真实设备矩阵验证切换、重连、Synthetic Clock 连续性和诊断串联；不能把固定 Stereo 产品策略冒充完整硬件能力发现。
...
- [x] `cpal_av_48khz_30min_recovery_v2` 以真实 48 kHz stereo CPAL callback、一次 validation-only stream recycle、Synthetic Clock fallback、新 generation 相位交接和最终 generation 连续 30 分钟消费为同一不可拆门禁；主音频流必须真实覆盖观察区间，容器时长或 EOF 补零不能替代 callback evidence。同期要求 Headless GPU presentation、Ready ≥99.5%、恢复稳定后零额外 loss/underrun/recovery/静默替代、Audio/Synthetic proven A/V phase error ≤20 ms、源缓存 ≤256 MiB 与完整产品进程树内存收敛。提交 `78a4d99` 的 2026-08-06 Release 运行自然完成：最终 generation 2 消费 1,801.153 秒，受控 recycle 后 Audio→Synthetic 为 66 μs、新 stream 打开 94.596 ms、恢复 124.280 ms；生命周期增量精确为一次 open/loss/controlled-recycle，backend loss 与 deactivation failure 均为零。Viewer Ready 53,801/53,949（99.72%），148 次均为候选尝试后的 `DroppedLate`，最大连续 2，GPU 精确呈现 53,949 帧；Audio Device/Synthetic proven phase 最大分别为 13.299/1.554 ms，零 underrun、替代与 recovery。源缓存 122,880,000/134,217,728 bytes，零 decode failure/oversize；完整 4 进程树 1,803 次采样全部完整，Windows Private Commit 峰值 745,394,176 bytes、稳态增长 1,457,603 bytes、post-stress 744,669,184 bytes，全部合同通过。该证据只关闭本 Windows 参考机的版本化门禁；多设备/驱动、冷启动/随机设备重启、声学 loopback、8 GiB 和 Linux/macOS 实机仍是独立资格矩阵，不能由这一次运行代替。

### 效果、动画、标题与转场

- [ ] 先交付少而完整的算子：Transform/Crop、Opacity/Blend、Primary Color、LUT、Gaussian Blur、Sharpen、基础 Mask、Cross Dissolve、Basic Title。`visual-authoring-roundtrip-v1` schema v10 已把 Gaussian Blur 2.75 px、Sharpen 0.45 与基础 Mask（稳定 Mask/shape-key 身份、静态 scalar 参数、Clip-local shape animation）通过正式产品 Action 加入同一 Hero Clip 的 `Primary → LUT → Blur → Sharpen` 有序栈；每次作者变更精确推进一次 Generation/Revision，Sharpen 参数另经 Undo 恢复规范 0 值并由 Redo 恢复，耐久关闭/重开后稳定 ID、参数与顺序不变。Preview/Export 从同一重开作者状态分别编译出相同图签名且各恰有一个 Blur/Sharpen 节点，Headless float-linear 像素在保存前后完全一致。该证据关闭产品作者与集成回归地基，不替代独立 Blur/Sharpen 数值/视觉 reference、真实媒体/嵌套、GPU backend、Viewer 几何手柄、路径编辑或独立 Mask 视觉 reference，因此整项保持未完成。
- [x] Basic Title 已作为 current document v25 的封闭 `ClipContent` 贯通持久化/校验、菜单创建、Inspector/Undo、Clip-local 视觉自动化、共享 Render Plan、working-linear straight-alpha、效果/Transform/Mask/合成、Cross Dissolve 端点、嵌套、Preview 与 Export。字体 bytes/face 参与输出身份，缺失字体或未声明 fallback 失败关闭；当前 roundtrip 覆盖产品编辑、插值、单事务 Undo/Redo、保存重开和 Preview/Export 同义，但不替代完整 Golden、独立排版 reference、缺字/长文本、多语言和压力验收。
...
- [ ] 对每个已发布 Alpha schema 提供只读旧 fixture、事务迁移、current document v25/library v5 重开与再次保存；未知/未来 schema、磁盘满、权限失败或中断必须不覆盖源并保留 Recovery Authority。验收包含幂等升级、失败重试、恢复冲突、备份发现和可观测诊断。
...
- Prepared Visual Program/Frame Closure/Range Closure、精确 Effect Execution Modes/Demand、受限异构 tracer、缓存复用准入、Preview/Export 动态 preflight、稳定媒体持久化契约、跨执行域资源决策和作者性能工具均已进入唯一生产路径；被替代的 Preview/Export 作者 walker、进程全局 Effect 编译/帧缓存、平行 plugin-contract registry、Core Render Graph 与未接生产的旧 renderer pipeline 已删除。平台契约已按 desktop/display/memory 拆成独立 Interface Module；原生文件对话框明确区分 Selected、用户 Cancelled 与 Adapter 失败，文件管理器启动失败也进入同一工作流错误路径。实现侧的 memory/process-tree/scheduling/global-pointer/display/eyedropper Locality 与媒体/Renderer 侧 D3D12/Metal/Vulkan 原生视频 Adapter 也已进入生产组合。脱离当前 Renderer 设备的平行 platform GPU probe 和未消费 shell Interface 已删除。剩余证据是原生 CI、设备 reference 与 M1 实机门禁，不能从代码存在推断通过。
...
- 本轮合并基线已用同一优化构建和 source attestation 完成 schema 10/protocol-v1 的 5/30/120 分钟 × active-heavy/project-heavy 作者矩阵，产生连续 **6 reports + 1 completion**，completion 证明 source 未变；JSONL SHA-256 为 `8611494cb0f590a5f0c142722c6f647f684e3794b3a8d5bda26ee6b0e95f2e0e`。六格全部通过 reference v4、Locality v3、History 与 Windows Private Commit：project median 满足 `<= 2 × active median + 5,000 us` 且 project p95 未超过 reference budget；两类 History endpoint、scope/source mismatch、unaffected payload、no-op/touched-Track COW locality、200 条深度、120 分钟 precompose Undo/Redo 和正常路径无 oversize/barrier 均由同一协议验证。retention-disabled、oversize、barrier 仍只由专用 probe 判定；Windows Private Commit 峰值/增量与 logical History charge 保持独立，Working Set 只作诊断。后续任何 `crates/` 源码变化都会使该 source attestation 失效，最终候选必须重跑；其他平台也必须建立绑定其 typed private-memory metric 的独立基线，不得复用或改名 Windows 门禁。若未来出现主导成本，只能深化现有 Authoring Session/Undo Implementation，不派生第二套 delta command 模型。
...
- 以已经接入 Preview/Export UI 无关生产调度的 bounded CPU-F32-DAG→GPU-F32-suffix route 为执行基线：`PreparedHeterogeneousEffectRoute` 已在媒体物化前冻结图、extent、budget、source-closed CPU dispatch DAG、一个或多个有序 CPU frontier upload 与 GPU dispatch/release schedule，Preview/Export worker 只绑定像素和执行。CPU Implementation 直接消费唯一 graph-value plan 的 materialization input/output，按精确 use-count 对 fan-out clone、last-use move、synthetic MaskSource 与 Blend/MultiInput/Mask join 做 live-set 管理；所有 transfer source 保留到原子 CPU completion，copy/kernel/Path-BVH/Mask raster 共享 attempt checkpoint，并把优化后的峰值帧数、Mask retained geometry/row scratch 与 kernel scratch 纳入 Effect Session。Renderer 按 boundary 顺序验证每个 materialization、non-converting value format 与 token 后上传，后续 upload 失败会移除此前未发布资源。SceneLinearRgb 上传为 Working，AlphaMask 上传为 `NonColorData/AlphaMask`，不经过 OCIO 或伪装为 RGB。融合容量内的 GPU 线性 point tail 保持单 pass，超出容量则拆为精确准入的 point dispatch；共享上传值的 GPU 分支既可通过二元 Blend，也可按作者顺序通过 MultiInput 左折叠任意 canonical scene-linear BlendMode，authored opacity 和完整 64 位 frame seed 由每一步共享 dispatch 保留。`N >= 2` 输入 MultiInput 的 `N - 2` 个私有中间纹理不进入 semantic graph；单输入则使用独立、位精确的 texture copy 保留 semantic materialization lifetime。两者均与最终输出进入同一物理 recording bytes/texture-count authority，并在录制失败时清理。CPU MaskSource→GPU Mask 同样只使用唯一 graph-value plan，单 pass 保留 RGB 并实现 Add/Subtract/Intersect/Difference 与 inversion。Gaussian→Basic Correction/Grain、CPU fan-out/Blend→GPU point tail、generated CPU Mask/Mask→GPU point tail、两条独立 CPU spatial 分支→GPU Screen join、CPU MaskSource→GPU Mask、GPU fan-out/Screen join、三输入 SoftLight GPU join 与单输入 identity copy 均有 scalar reference；后五项另有真实 GPU token/live-set/resource parity，GPU Mask 覆盖八组操作，copy 还要求逐位 Float32 一致。共享 compositor 还对全部 canonical BlendMode、straight alpha、阈值/通道并列输入和 Dissolve 高低位种子完成真实 wgpu→CPU Float32 parity。wgpu Adapter 以同一份物理要求准入全部 upload bytes、单 command buffer 实际非别名的 recording bytes 与 texture count；Preview、Export、Viewer 及 recorded evidence 均不再把抽象 live-set peak 冒充物理所有权。Preview 的 canonical closure 现先冻结全部 child binding，再通过原子 payload-finalization Seam 建立逐 placement 路线账本：嵌套 child 自身仍必须走完整 CPU 物化，但父级 Nested Clip 可把该 CPU working frame 作为异构前缀输入，route extent 严格取 child 的执行 raster 而非父画布；缺失 binding/extent 在任何媒体 Adapter 前失败。生产回归同时证明父级 Nested Gaussian→GPU point tail 可准备、异构 child 不得冒充完整 CPU 物化。Renderer-owned `HeterogeneousCpuPrefixSource` 现同时承载既有 CPU working frame 与程序化 Solid Color；Solid 规划只冻结 extent+straight-alpha working color，Preview/Export 在 route/source grant 验证后才由同一 serial executor 分配全栅格，不伪造媒体帧、不各自解释生成源。Preview 不调用媒体/标题 Adapter；Export 在不足 source+frontier 授权时不开始 CPU prefix，并以真实 GPU readback 对 CPU Float32 reference。下一步继续 Transition endpoint 独立 completion、Adjustment accumulator 同步/readback、显式颜色域转换 Adapter、外部 lane executor，以及 temporal×heterogeneous 组合；GPU 后再切换 backend 或图内 readback 仍保持阻塞。现有 Preview worker/completion lease 与 Export attempt-local upload/dispatch/wait/readback 不能被第二套 runtime 取代。Linear Stage Placement 不参与执行；缺少 exact mode、已声明 transfer、具体 Adapter 或可证明 live-set 时必须在像素执行前类型化拒绝，异构提交开始后不做隐式整图 CPU fallback。
- 异构路线身份与批次资源权威已经收回深层 Module：Effects 冻结覆盖 dispatch/transfer/release、lane/backend、完整 value format 与 operation kind 的版本化 opaque shape identity，并排除 extent/作者参数；Renderer 通过 `PreparedHeterogeneousEffectRoute` 暴露该身份、规划资源证据及全部 frontier retained bytes。Export route ledger 只比较这份 route 身份，不再遍历节点重建 fingerprint 或把非 RGB frontier 当成帧契约错误；Preview batch 按每个 input 加全部 frontier 精确准入，8 GiB 档位只承诺一个 input + 一个 frontier 的 UHD 基线，更宽 frontier 必须消费额外授权。
