...
- [ ] 用户可见 Component 选择与显式重绑已贯通：Inspector 以稳定 Edit ID 在 Asset Component/嵌套公开输出域内选择并进入 Sequence Undo；资产级“重新探测候选→显式重绑”分离自动证据与用户意图，保留逻辑 ID，更新后会在当前 transport anchor 失效并重建音频 Runtime。Canonical authored mix matrix 的持久化、验证、稠密调度执行、嵌套布局边界和播放/导出交付分层已完成；仍须补齐完整矩阵编辑器/预设、custom media layout probe、plugin bus/layout negotiation 与非标准设备/编码协商。Processor Host 已把 semantic preservation、prepare-time Resolver/Factory、Session 实例、mode/algorithmic-latency/tail/state/scratch、显式 lookahead/Session 字节预算、主 Bus/辅助输入查询和公共参数 batch 分层；内建定义已独立到深 Module，Gain 与状态型 Sample Delay 走同一 Host，非零延迟测试 Adapter 继续贯通 PDC。随后实现隔离的 VST3/CLAP Adapter、deadline/crash containment、state chunk 与真实 bus/layout 协商，并基于已执行的 typed Route/send 边实现 authorable processor sidechain、true-peak/标准响度和 Rack/Automation/Meter 编辑 UI/命令。Sample Delay 的可听延迟明确不是可补偿 latency；首个生产级 lookahead/group-delay Processor 必须复用已统一的 Playback/Export/嵌套公共输出归一化与 stage-time 语义，并同时交付真实 state、entry/deadline/PDC 与嵌套正向证据。GPU 仅可作为声明固定批量延迟、计入 PDC、设备丢失/状态切换可控且实测优于 CPU SIMD 的可选 backend。callback 实时安全、underrun 与 A/V drift 继续由真实门禁证明。
