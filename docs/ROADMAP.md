...
4. **声明能力和视觉执行能力可能分离。** Basic Title 与 Cross Dissolve 已闭合主路径，但其完成不能外推到其他仅有类型、属性或 definition 的效果；路线图仍须逐项核对产品选择、Render Plan、backend、Preview/Export 和真实项目证据。
...
机器可读合同已升级为封闭 schema v3：未知或拼错字段失败关闭；Sequence 明确为 3840×2160、25/1、square pixel、progressive、linear Rec.2020 working、Rec.709 legal 输出、10-bit 交付默认值和 48 kHz stereo；H.264/AAC 与 HEVC Main10 门禁绑定稳定内建 preset ID，并固定 profile、分辨率、位深、chroma、pixel format、range、Alpha、音频码率、Rec.709 CICP 与静态 HDR metadata 缺席策略。`foundation-audio-authoring-v1` 使用规范 PCM fixture 和普通产品导入/时间线/Inspector/Undo/Redo/耐久保存接口，记录完整 Sequence settings、原生 Stereo、每个作者事务的 Session/Generation/Sequence Revision、持久化 request/archive identity 与重开后类型化作者值，只关闭 open、undo-redo、save-reopen 与 Clip audio gain/pan/fades。`editorial-transport-v1` 把 attested AAC 作为真实时间线依赖，通过普通媒体导入、Timeline Action、选择、Transport 与 Playback Evidence 完成 Overwrite 左片段保留、Split、Ripple Delete、跨两条显式 ripple Track 的十帧专业 Insert、三次 Pointer-drag Scrub、Settled Accurate Seek，并证明 Play 由 Synthetic Clock Master 单调推进；Insert 证据同时固定新 Clip 范围、两轨下游位移和单次作者 Generation/Sequence Revision，其他结构编辑也继续遵守同一事务边界。该短窗只证明 Golden 操作及后置条件，不替代 30 分钟 CPAL/GPU/内存/A/V 专业门禁。`proxy-relink-v1` 使用项目自生成且明确不具备色彩参考资格的 H.264 High Rec.709 fixture，通过普通导入、实例级 Proxy Generation Service、canonical Preview media resolver 与产品资产 Action 证明 Proxy→Original→Proxy、结构化离线、同一 `AssetId`/Clip/名称重连、Asset Library revision 单次提交、旧路径代理不可复用和 replacement 代理重新生成；代理模式切换是 Project 作者事务，重连本身不是 Sequence 编辑。`generated-delivery-roundtrip-v1` 使用普通产品 Action 创建 Solid Color 与 PCM placement、精确 Trim 25 帧、设置 Transform/Opacity，真实导出 H.264 High 8-bit 1080p 与 HEVC Main10 10-bit 4K，生产 `Completed` 必须先证明 MP4 mux、codec/profile、位深、fps、CICP/range、HDR 静态 metadata 缺席、48 kHz Stereo AAC 与时长，再通过普通媒体 worker 重导入并保留 H264High/HevcMain10 类型化 profile。`visual-authoring-roundtrip-v1` 不依赖外部素材：产品 Action 创建相邻 Solid Color、Cross Dissolve 与 Basic Title，正式属性 mutation 写入 Hold/Linear/Bezier 曲线，并由 Curve Editor 产品 Action 按稳定 Parameter/Keyframe 身份移动一个 Bezier key；每个用户意图、Undo 和 Redo 必须只前进一次 Generation/Revision。Headless Preview 使用递归时间线执行、真实系统字体栅格和 float-linear CPU 合成，并与 Export Render Plan 对照标题语义和转场系数，耐久保存重开后还必须保持被编辑 key 的 ID/插值、raster signature 与像素哈希。五个切片均已在本地真实通过，但生成画面不是独立色彩 reference，视觉切片也不冒充完整关键帧产品 UI；各报告只能关闭自身声明的局部义务，HLG/sRGB Alpha 合格参考、Primary Color/LUT、嵌套/恢复与连续三轮顶层 Golden 仍未完成。

顶层 `GoldenAcceptancePlan` 现把 fixture/operation/content/export 全部义务编译成确定性账本，并明确报告未规划与未绑定项；计划与五个切片报告都固定 `complete_golden_project: false`，只有未来单一 Project 身份的总运行协调器可在完整观察后声明顶层通过。AAC 编辑/Transport、专业 Insert 与 H.264 Proxy/Original/Offline Relink 均已有切片和真实执行证据；操作账本已无未规划项，当前只剩 HLG Main10/sRGB Alpha 两类图片角色、Primary Color/LUT，以及两类合格图片 fixture。专业 Insert 使用独立的显式 target/ripple scope、链接组、锁定、Transition 和 Sequence-time automation 合同；局部碰撞模式已经更名 `ClipOverlapMode::PushForward`，避免再被误当 Insert。Rust 与 PowerShell 校验器均以 schema v3、三轮通过要求和 slice export 引用失败关闭，不能再由旧脚本或测试进程退出码虚构完整性。

单 Project 执行地基现已开始收敛：`GoldenProductWorkflowDriver` 持有一个真实 App composition、固定 `ProjectId + project path` 与该运行的全部阶段 Sequence；Project-scoped 作者事务只要求 Generation 精确前进并允许活动 Sequence 改变，Sequence-scoped 事务还要求活动 Sequence 不变且其 Revision 精确前进。普通 CI 已证明同一项目中新建第二阶段 Sequence、耐久保存、关闭和重开后 Project/path/两个 Sequence 均不漂移且只有 Session 身份更新。foundation audio、visual authoring、editorial transport（含多轨 Insert）、proxy/relink 与 generated delivery 五个现有真实切片均已成为可组合阶段：门禁在同一 Project 中依次保留 PCM 作者内容、Transition/Basic Title、AAC 编辑与 Transport 状态、同一 Asset/Clip 的 replacement 路径与代理意图、H.264 High 与 HEVC Main10 导出/重导入，再经最终 durable reopen 证明五个完整 Sequence 快照、Project/path、重连资产和两个成品 profile 均不漂移。顶层 Coordinator、真实色彩/嵌套/恢复义务与连续三轮完整运行仍明确阻塞。
...
- [x] Golden 单 Project 工作流已有生产接缝：Headless driver 固定一个 `ProjectId + path + AppState`，区分 Project/Sequence 两类作者事务证据，并在初次 open、阶段 Sequence 创建和 durable reopen 前后失败关闭身份漂移；五个现有切片均为可复用阶段。真实 PCM/AAC/H.264+系统字体+FFmpeg 组合门禁在同一项目内完成五阶段、两次代理生成、离线重连、两种交付和重导入，并经最终 reopen 对照五份完整 Sequence 序列化快照、同一 relinked Asset/path/proxy intent 及 H264High/HevcMain10 成品。恢复与其他缺失义务、顶层 Coordinator 和连续三轮仍须完成。
...
- [ ] durable worker、SQLite snapshot、Session/generation completion、dirty/manual/autosave baseline 和原子 manifest 已完成；补齐真实项目上的异常退出恢复选择、来源/时间/冲突 UI、磁盘满/权限失败、保留策略、dirty/close 与另存为三轮 Golden Project 验收。
