...
- 任务调度不得成为只转发到现有 worker 的浅模块。当前 Frame Work Broker、UI 无关 Waveform Analysis Service 与 Thumbnail Execution Service 已作为三种独立 Implementation 消费统一语言；代理和导出迁移时仍须证明各自的深策略，不能退化为共享池调用。
