...
- [ ] `ProcessMemoryProbe` 已建立无 OS 调用的接口与 Windows Process Status Adapter；专业门禁以固定 cadence 聚合 5–10/25–30 分钟 Private Commit 稳定窗、4 GiB 绝对上限和各门禁声明的 terminal stress 静止后收敛，unsupported/缺样/探针错误失败关闭；视频 v4 还要求主视频流时长及声明帧数覆盖，且专业 cadence/timeout/latency/readiness/hardware 阈值不读取 smoke 调参。原生 `AVFrame` 可能钉住整个硬件 surface pool，现已在停止传输、最终注册 GPU Viewer 输出精确绑定当前完整 Preview generation 且 Broker/执行意图静止后，仅释放 media payload/pin，并由 worker idle 销毁 FFmpeg Session；GPU Viewer 输出、stale pin 与失败记忆继续有效，后续同 generation 查询不会自激重解码。GPU 输出发布可能早于完成 lease 的最终 Broker 结算，因此 Runtime 在 stopped generation 真正旋转前、旧输出仍具 exact 证明时再次尝试释放；一旦旋转，旧输出只能作为 stale presentation，不能再授权源驻留决策。准确 seek 的 Long-GOP 非参考前缀优化恢复完整解码后才允许选帧，submitted packet 同样计入预算，专业门禁新增 temporal approximation 必须为零。2026-07-20 的清洁 revision Reference Run 中，真实 CPAL/A/V 30 分钟门禁通过（零 drift/underrun/recovery，整进程 settled growth 约 85.6 MiB），但视频在完整 30 分钟连续播放后进入 seek 时暴露 D3D12VA still 会话在 codec 内不可协作阻塞；只把 GPU still 改为每次销毁/重开 decoder 的后续清洁复验仍在第二个精确 still 阻塞，证明该临时策略不是根因并已删除。当前实现把 Preview Frame Store 与预取的 native-resource 上限统一为 8 帧；兼容的 mode-local still decoder 在成功或合作取消后保留，但每个随机目标仍执行索引化 exact seek + codec flush，取消部分输出不得缓存，源/指纹/设备/几何/色彩契约变化则先销毁旧 session。2026-07-21 的短真实 4K Main10/100-seek release 复验完成 300/300 Ready、402/402 native P010 GPU 输出、52 次精确 still（2 次 session open/50 次 reuse）、零近似/零 budget failure、取消 request→checkpoint 最大 292 µs、latest-wins 后零 pending/queued/in-flight/媒体表面驻留；跨区 seek 总时长由反复重开策略的约 15.3 秒降至约 11.2 秒，整项 smoke 全绿。确定性 1812 秒视频与 1835 秒音频配方、机器资格验证、完整 Video+Audio 编排及 evidence bundle 已闭合；仍须在满足 profile 的固定 Windows 参考机对当前清洁 revision 完整复验 30 分钟视频门禁，确认长时后不再阻塞并据实校准版本化阈值。
