...
- [ ] 三平台生产路径已经能够把 D3D11/D3D12、VideoToolbox/CVPixelBuffer、VA-API/DRM PRIME 输入同一 GPU YUV→working 执行链；下一门槛以 4K23.976–60 HEVC Main10 Long-GOP 为输入，在支持设备上观察逐帧 hardware→native surface→GPU YUV/working 证据。Windows 完成 M1 发布资格；macOS/Linux 先通过原生 CI，实机资格可晚于 Windows。缺设备、格式、可信 DRM layout、同步或预算时必须自动选择 CPU transfer、低分辨率或代理并显示原因，不能错误读回、改色或伪报 zero-copy。当前 `0175f35` 的 4K25 视频门禁已在合格 16 GiB Windows 参考机以 30 分钟 cadence 通过：45,003 个媒体层全部为原生 P010 hardware execution，GPU import 45,105 个 sample 全部闭合，CPU transfer/readback/fallback/blocker 均为零；进程树 Windows Private Commit 峰值 1,215,262,720 bytes，settled growth 17,589,808 bytes，post-stress 为 942,784,512 bytes。该单 gate 仍不代表 23.976/50/60、多 GPU/驱动或完整 A/V baseline，因此本项不勾选。
  2026-08-06 又以本地临时 4K59.94 HEVC Main10 BT.2020/PQ 长流在当前生产实现完成 107,895 帧真实 cadence：第二轮 107,894 Ready/1 Stale，50 次 warm seek p95 17.447 ms、52 次 accurate seek p95 112.441 ms、100 次 supersession；专业合同观察 100% native P010 hardware、107,998/107,998 个 renderer import timing sample、零 missing/drop/mismatch，完整进程树 Private Commit 峰值 1,122,627,584 bytes、稳态增长 61,044,832 bytes、post-stress 1,037,647,872 bytes。两轮最终只被旧外层 `viewer_gpu_execution_classification` 错误拒绝：该守恒式遗漏了 107,894 个合法 `PreparedSuccessor` disposition，同时独立 presentation coverage 已完整。门禁现分别验证 `PublishedCurrent | PreparedSuccessor | Released | TerminalRejected` execution 守恒和 unique presentation coverage；该报告是修复依据，不冒充修复后 clean revision 的自然通过，正式 59.94 门禁仍须重跑后才能更新资格。
...
- [x] 最终候选的 `windows-alpha-golden-v12`/schema v4 完整工作流按第 5.2 节连续 3 次通过；每轮使用独立 run/Project identity，七个 stage 绑定同一 Hero，最终仅保留 Hero 与一个强引用 child，并观察全部切片、CFR/VFR signed-retime 反事实、物理 PTS 区间、Preview/Export/Headless/重导入证据。当前源码重建后的正式运行 `20260805T163443Z-complete-golden-58886bac` 在 `Pr/All` 素材身份检查后连续 `3/3` 通过，三个进程自然退出且未触发 terminal-report recovery；aggregate SHA-256 为 `20fa59a873180594c927ec1566f4bef225a610eaf6310da0c655e271c0a16633`。该证据只关闭版本化完整 Golden 重复门槛，不替代 4K/长时 A/V、内存、真实设备或独立绝对色彩 reference 门禁。
