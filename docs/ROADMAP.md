...
- [x] `PlaybackEngine`、`FrameWorkBroker`、`PreviewFrameStore`、Monotonic Runtime Clock、Playback/Frame Cancellation Evidence 与 Headless GPU Adapter 已从 UI 收敛；Broker execution lease 是优先级/访问类/worker lane 驻留证据的单一事实来源，App activity 原子计数已删除；访问模式/Broker Adapter/有界 job transport 位于 `app::preview_access_mode`，Window 与 Headless 的“单次 Demand 采样→完成/过期→终态 Delivery→预卷”统一在 `app::playback_preview`；预览内部的 timeline evaluation、media adaptation、decoded/native payload 与单一 lazy CPU working adaptation、Viewer plan、working-linear CPU composite/raster boundary、media execution、presentation 与 diagnostics 已按行为所有权成为私有深 Module，最终 GPU/Raster/stale/CPU output 仲裁只在 presentation Module，且未扩大跨层 Interface。
...
- [ ] source Seam 已从逐样本调用改为每个 Contribution 每块一次的索引化交错读取；semantic IR 已 prepare 为稠密 schedule、连续 Route/Contribution 区间、Transition binding、精确 sample span、预分段 automation event span 与 liveness scratch，CPU scalar reference/SIMD kernel 及 1/8/32/64 轨负载矩阵已建立。Prepared latency solver 已在每个 Contribution→Track 与 port-specific Route→Bus/Output 汇合点计算补偿，累计 pre/post rack 与 Program Output latency；嵌套 Runtime 自底向上传递实例级 child-output latency，缺失依赖失败关闭而不猜 0。当前可执行 Gain 仍严格为零延迟；非零 processor 必须同时交付真实 processor state、预分配 compensation line、block-partition PCM 证据及 discontinuity state entry 后才能准入。继续扩展多 Bus/真实 processor deadline 矩阵与插件参数事件 batch；补齐 channel layout/组件选流/重采样，再实现隔离的 VST3/CLAP host、send/sidechain、meter/loudness 和编辑 UI/命令；GPU 仅允许作为声明固定批量延迟、计入 PDC、设备丢失与状态切换可控且实测优于 CPU SIMD 的可选 processor backend；callback 实时安全、underrun 与 A/V drift 继续由真实门禁证明。
