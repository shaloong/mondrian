...
| 渲染与色彩 | Preview 与 Export 的生产 Timeline 求值已共用按 `SequenceId + SequenceRevision` 绑定的 `PreparedVisualSchedule`：准备期完整验证，按半开区间索引 Clip/Transition 并保留作者 Track/Clip 次序；Preview 的长期缓存覆盖根/嵌套 Viewer、预取与 preroll，Export 的 job Session 缓存覆盖根/嵌套逐帧渲染，诊断遍历也经短生命周期缓存消费同一准备表示。直接 Sequence 求值保留为语义 reference Adapter，转场边界等价与 10,000 Clip 稀疏查询有自动门禁及 Criterion 基准。Core 中无调用者的并行 Render Graph 已删除，效果图只由 `mondrian-effects` 定义/编译，frame plan 只由 renderer 求值。working-space 合成、OCIO CPU/GPU 路径、结构化色彩/显示诊断、golden 测试、预览/导出报告对比、Windows 显示探测和 fail-closed 逻辑较深入 | Prepared Schedule 当前只消除 placement 全量扫描；effect graph 的跨帧准备/复用、Temporal/ROI 能力与缓存安全仍须作为真实执行语义设计，不能用空 capability 字段冒充。仍有 CPU/读回路径与真实显示 payload 限制；常见 Log/HDR 必须补齐参考样片端到端证明；Windows HDR 监看不能提前宣称稳定 | L1+ 架构，继续执行准备化与符合性收敛 |
