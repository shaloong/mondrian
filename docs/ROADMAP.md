...
| 时间线编辑 | 多轨、移动、分割、普通 Trim、Ripple Delete、Overwrite、Roll/Slip/Slide、跨轨移动、锁定、吸附、多选和嵌套序列已有实现与测试；`ClipOverlapMode::Insert` 仅提供单轨局部碰撞顺推，明确不计为专业 Insert Edit。Clip 已用封闭 `ClipContent` 删除矛盾 kind payload，并增加 Sequence-local Basic Title；链接改为可承载任意成员的 `ClipLinkGroupId` 集合；复制/切割/覆写片段/序列复制会 fork 完整作者身份图；显式视频 Transition 已有强端点、精确范围、同轨不重叠不变量和不裁切的双源 handle demand；Cross Dissolve 产品命令会用媒体/嵌套真实范围默认拒绝短 handle，显式缩短与创建/删除均为单次 Undo 事务；时间线已能在精确相邻未锁定的视频 cut 创建、按稳定 ID 选择/删除、拖动两侧范围并显示当前 handle 失败诊断 | 专业 Insert Edit 必须先统一 Track Targeting、Sync Lock/ripple scope、插入点拆分、链接组、锁定、Transition 与 Sequence-time automation 的原子迁移/拒绝；Lift/Extract、显式 Link/Unlink 产品 UI、reverse/freeze/time remap 仍未闭环；Cross Dissolve 和 Basic Title 已有局部 Golden，但仍缺真实媒体/嵌套/独立视觉 reference 的顶层验收 | L1+ 作者、命令与基础 UI闭环；专业 Insert 仍是明确缺口 |
