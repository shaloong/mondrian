...
| 渲染与色彩 | Preview/Export 共用按 Sequence/Registry revision 与作者 fingerprint 绑定的不可变 `PreparedVisualProgram`；逐帧 `PreparedVisualFrameClosure` 固定 nested time、canvas/color、Transition/temporal binding 与 instance path，区间 `PreparedVisualRangeClosure` 固定 selected-range 的媒体、Transition 和字体依赖。`CompiledEffectGraph` 是唯一生产 IR，动态 topology/cache 只驻留于调用方 `EffectExecutionSession`。有限历史 CPU-F32 tracer 已执行精确输入 ROI：局部缓冲保留完整画布坐标，有限核使用实现推导 halo，resident value/provider/output/kernel scratch 在分配前受同一 Session byte grant 约束；Blur/Vignette/frame-seeded Grain 与完整帧 crop 有数值对等回归。异构提交开始后的取消、GPU 或证据失败是本次 attempt 终态，不隐式整图 CPU 重跑 | 当前 temporal 与异构执行仍是 bounded tracer，Preview/Export 的完整画面交付也通常请求整帧，不能把局部执行器能力冒充产品级自动切片。后续必须以有限历史 Processor、扩大 ROI Processor、嵌套和 fan-out/join 为输入，闭合全帧 tile scheduling、exact temporal window、halo/full-frame/unknown、continuity、resource lifetime、seek/cancel 与 CPU/GPU reference parity；缺少 exact mode、live-set 或 Adapter 时在像素执行前阻止。Log/HDR 仍需独立 reference | L1+ 唯一 IR、Prepared closure 与精确 ROI scalar 边界闭合；通用 Temporal/ROI/异构执行未完成 |
...
- 已让 CPU-F32 scalar ROI executor 消费同一 demand：精确输入 tile 保留完整画布坐标，有限核消费实现推导 halo，full-frame/unknown 证据不降级，resident value 与 kernel scratch 在分配前纳入 Session byte grant；Blur/Vignette/frame-seeded Grain 已有 full-frame crop parity。下一步是在不改变该语义参考的前提下增加完整画面的调度级切片、通用 DAG live-set 与 CPU/GPU parity；不得用裁剪后的错误像素换取性能，也不得把 executor 的局部能力写成 Preview/Export 已自动分块。
- 以已经接入 Preview/Export UI 无关生产调度的 bounded CPU-F32→GPU-F32 tracer 为执行基线，沿唯一 `CompiledEffectGraph` 深化通用 fan-out/join、更多 placement、颜色域转换、外部 lane executor，以及 temporal×heterogeneous 组合。现有 Preview worker/completion lease 与 Export attempt-local upload/dispatch/wait/readback 不能被第二套 runtime 取代。Linear Stage Placement 不参与执行；缺少 exact mode、已声明 transfer、具体 Adapter 或可证明 live-set 时必须在像素执行前类型化拒绝，异构提交开始后不做隐式整图 CPU fallback。
