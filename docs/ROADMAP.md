...
- [ ] 为 Clip/Track/Bus/Program Output 的 Rack、参数自动化和 meter 提供同一套稳定 Scope/Processor/ParameterId 驱动的编辑 UI/命令。共享作者地基已使用 `AudioProcessorRackEditRequest` 封闭地址与编辑代数：Scope 或类型化 Channel Strip/Pre/Post 地址、稳定相对 placement、Processor/Parameter/Keyframe ID、共享 Scope 锁轨准入、完整 Audio Program 原子校验、单次事务、无操作零提交及 Undo/Redo 均有跨 Timeline/App 证据；保存重开继续由 current document v23 的 Processor Schema/精确曲线回归覆盖。Clip Processing Scope 已进入真实 Inspector：公共投影按 Scope 去重并扫描全 Sequence 锁定绑定，产品目录通过独立 typed intent 在 dispatch 时创建规范 Gain/Limiter 实例，不在 UI 重投影时分配作者 ID；稳定 ID reorder/remove/bypass 和无关键帧参数的 schema 驱动编辑走同一 Rack 事务。已有关键帧只显示自动化计数且禁止把 fallback 值冒充播放头值。首个 lookahead limiter 已复用公共 Processor Host，并通过 state、entry/deadline/PDC、播放/导出一致性、callback 实时安全和多轨负载矩阵；仍缺 Track/Bus/Output Mixer、精确 owner-time curve/meter UI、UI 交互 Golden、外部插件 Adapter 与真实设备长时证据，因此本项不提前勾选。
