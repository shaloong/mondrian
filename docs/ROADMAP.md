...
机器可读合同身份已升级为 `windows-alpha-golden-v9`、封闭 schema v4：未知字段失败关闭；Hero Sequence 固定 7,500 帧、3840×2160、25/1、square pixel、progressive、linear Rec.2020 working、Rec.709 legal Program Output、10-bit 交付默认值和 48 kHz stereo；H.264/AAC 与 HEVC Main10 绑定稳定内建 preset ID 及完整 signal/probe 合同。七个切片仍只通过普通产品接口执行，并分别保留 PCM/Clip 音频、Editorial/Transport、Proxy/Relink、短窗交付、视觉作者、Recovery/Nesting 与 HLG/Alpha 的局部证据；Foundation Audio、Visual Authoring、Editorial/Transport 与 Generated Delivery 已真实进入同一 Hero Sequence。Editorial 只绑定两条完整 pristine 音轨，定向 Split/Insert 消费产品返回的类型化身份 receipt，并在每次 scrub、settled seek 与 play 上通过生产 Preview Runtime 的 GPU 优先/CPU Raster fallback 仲裁消费精确 Frame Presentation Ticket；fallback 路径会如实写入证据，不能冒充 GPU。Delivery 复用 Foundation 的 PCM Placement，在 `150..175` 非零 Work Area 上通过产品 Action 完成 Trim、Transform 与 Opacity，durable reopen 后才执行 H.264 High/HEVC Main10 导出及普通媒体重导入；Program reference、成片解码像素、生产音频 Program Runtime、AAC `AudioSourceReader` 回读、精确 stream PTS/duration/time-base 与两种编码的一致性共同形成闭环。其余局部证据不能因共用一个 Project 就自动成为完整节目证据。生成画面、identity LUT 与 Solid Color 嵌套只提供回归一致性；HLG slice 当前只有编码码值和结构性工作空间检查，不替代完整绝对 HLG/PQ/Log 独立色彩 reference；短窗播放也不替代 30 分钟 CPAL/GPU/内存/A/V 门禁。
...
`GoldenProductWorkflowDriver` 仍持有一个真实 App composition、固定 `ProjectId + project path`，并使七个切片可复用为诊断阶段；其私有 Hero binding 锁定初始 `SequenceId`、完整设置和 Project Color Environment，Hero slice 只能复用该身份，诊断 slice 才能通过产品作者事务创建新 Sequence。Foundation Audio、Visual Authoring、Editorial/Transport 与 Generated Delivery 现共享 Hero；Proxy/Relink、Recovery parent/child 与 Color Media 仍形成四条诊断 Sequence，当前完整 Project 共五条。组合门禁在 Visual 前后比较完整 Hero 音频投影，在 Editorial 和 Delivery 前后分别比较其前序阶段的 Track-owned/完整作者锚点，避免后续阶段以同 ID 但被改写的内容虚假保留。此前八条隔离 Sequence 的三轮运行真实证明了局部路径和共同 Project 生命周期，但没有证明它们在一条 5 分钟节目时间线上共同成立，因此该三轮不再计作 M1 完整 Golden。v9 会在重载荷执行前明确列出仅余七项 Hero 缺失义务并失败关闭：fixture 为 HLG Main10、Rec.709 H.264、sRGB Alpha；操作为 Proxy/Original Switch、Offline Relink、Autosave Recovery；内容为 Nested Sequence；交付已经没有未绑定项。下一步不是删除切片，而是让这些现有实现逐步复用 Hero；最终 durable reopen 必须保留身份、7,500 帧内容边界及全部关联证据。
...
- [x] Golden v9 的结构完整性由单一深 Module 负责：确定性全局账本与 Hero 账本分别计算必需、已规划、缺失与未绑定义务；Rust/PowerShell 同步校验 schema、Sequence role、Hero 身份、三轮要求和 slice export 引用。所有独立切片无论成功或失败都显式声明 `complete_golden_project: false`；当前全局账本完整，Foundation/Visual/Editorial/Delivery 义务已进入 Hero，但 Hero 账本仍明确 blocked，这正是防止“多个孤岛相加等于成片”的预期结果。
- [x] Golden 单 Project Coordinator 和监督器的机制已进入专用进程主生命周期：Headless driver 固定 `ProjectId + path + AppState`，私有绑定锁定 Hero ID、设置与 Project 色彩环境，并统一决定 Hero 复用或诊断 Sequence 产品创建；open、阶段绑定、Autosave Recovery 和 durable reopen 前后均失败关闭身份漂移。Coordinator 现还要求全部 Hero slice 的 primary `SequenceId` 唯一一致、最终 reopen 保留 Hero 身份且内容边界精确为 7,500 帧；stage-owned Sequence 可共享身份，嵌套 child 可额外存在。Foundation Audio、Visual Authoring、Editorial/Transport 与 Generated Delivery 已共享 Hero；完整投影/Track-owned 锚点证明后续阶段没有改写既有作者状态，五次真实 Headless presentation 证明 seek/play 输出而非直接注入 Delivery，非零 Work Area 的双交付/重导入证据证明导出不再来自隔离样例 Sequence。其余阶段尚未迁移，因此监督器应继续失败而不是复用此前隔离 Sequence 三轮的 `complete=true` 结果。
