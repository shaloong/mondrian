...
`GoldenProductWorkflowDriver` 仍持有一个真实 App composition、固定 `ProjectId + project path`，并使七个切片可复用为诊断阶段；其私有 Hero binding 锁定初始 `SequenceId`、完整设置和 Project Color Environment，Hero slice 只能复用该身份，诊断 slice 才能通过产品作者事务创建新 Sequence。Foundation Audio、Visual Authoring、Editorial/Transport、Generated Delivery、Proxy/Relink 与 Recovery/Nesting 现共享 Hero；Recovery 只新增合法的 Nested Composition child，仅 Color Media 仍使用诊断 Sequence，当前完整 Project 共三条 Sequence。组合门禁在 Visual 前后比较完整 Hero 音频投影，在 Editorial、Delivery、Proxy 与 Recovery 前后比较各前序阶段的 Track-owned/局部作者锚点；Proxy 锚点精确覆盖自己的 Track/Clip、重连后 Asset 与 proxy intent，Recovery 自身还比较完整 Hero parent 和 nested child 快照，避免同 ID 内容被改写后仍虚假保留。此前八条隔离 Sequence 的三轮运行真实证明了局部路径和共同 Project 生命周期，但没有证明它们在一条 5 分钟节目时间线上共同成立，因此该三轮不再计作 M1 完整 Golden。v9 会在重载荷执行前明确列出仅余两项 Hero 缺失义务：HLG Main10 与 sRGB Alpha fixture；操作、内容与交付已经没有未绑定项。下一步是把现有 Color Media 实现复用到 Hero，并在完整色彩语义不降级的前提下取得最终 durable reopen 与三轮证据。
