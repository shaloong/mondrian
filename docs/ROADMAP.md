...
- [ ] FFmpeg open/stream-info/seek/packet I/O 已接入 request-scoped interrupt，取消结果会保留首次观测 checkpoint 与 cooperative/FFmpeg-I/O 机制，外部 still 子进程可 kill/wait/join；loopback HTTP stall 已在 media Interface 与生产 Preview worker 两层证明 blocked input-open 会由真实 `AVIOInterruptCB` 有界返回并进入 Runtime/专业报告聚合，而不是只声明回调。每名有界 Preview worker 现在还持有唯一、无锁的 `PreviewDecodeExecutionObserver`：正式 diagnostics/Headless Evidence 可区分 input open、stream info、hardware device、codec open、seek/flush、packet read、codec send/receive、materialization、output lease 与 session retire，并以 request/progress sequence 证明观察一致性；同一快照还分别保留 callback poll、观察取消次数和最后取消所属 request，不能再把“已安装 callback”“callback 被调用”“callback 返回取消”“FFmpeg 调用返回”混为一谈。独立 sampler 只克隆只读 watch，以 100 ms 采样、进展变化/5 秒 heartbeat 稀疏写入并逐条 flush schema-v2 JSONL，主门禁线程或驱动调用不返回时仍可由外部进程取得最后阶段。Reference Playback Gate Supervisor 以计划内 45 分钟墙钟期限、异步双管道读取和完整后代进程树终止约束 Video/Audio 进程；timeout 必定失败并保留日志/进度证据，不能伪装 Broker lease 已完成或拥有 codec 恢复权威。播放域已统一 5 ms request→checkpoint、50 ms Playback/Interactive return、500 ms Still return 的 fail-closed 门禁。规范 corpus 已提供不承担色彩正确性声明的确定性 4K25 HEVC Main10 Long-GOP 配方，Reference Playback Run 会绑定 recipe/artifact/机器/报告身份；仍须在合格固定参考机用 pause/seek/close/quit、真实驱动以及 stream-info/seek/packet-I/O 或驱动阻塞取得最坏延迟证据，且 UI 线程不得 join worker。
- [ ] `ProcessMemoryProbe`、Windows Process Status Adapter、固定 cadence 的 Private Commit 平台、4 GiB 绝对上限、terminal-stress 收敛、主视频流时长/声明帧数覆盖及不可由 smoke 参数放宽的专业阈值均已进入 Video v4；原生 `AVFrame`、Frame Store、renderer copy fence、worker session 与 residency-family ACK 的独立退休证明也已接入。确定性 1812 秒视频与 1835 秒音频配方、8/16/32 GiB 机器资格、完整 Video+Audio 编排及 evidence bundle 已闭合；真实 CPAL/A/V 30 分钟门禁已通过，但 Video 门禁仍被非确定性的 FFmpeg/native 调用不返回阻塞，因此本项保持未完成。

  - 清洁 revision `4b57403` 的正式运行完成全部 44,994 个原生 D3D12VA P010 cadence Playback 帧后，第二个 exact 停在 codec；压缩墙钟运行又可完成 45,000 帧并追加 2 次 seek，证明累计帧数或 device 存活本身不是充分条件。清洁 revision `68d76aa` 的正式运行 `20260721T175344Z-local-windows-dev-01-990fd909` 再次完成 44,994 个 Playback 帧，但首个后续 exact 在取消 30 秒后仍不返回；禁用 FFmpeg frame threading 得到相同结果。另一次完整生产运行约在第 28,353 个 Playback 帧进入超过 11 分钟不返回的 codec 调用，证明故障也不是 exact-only。当前 native exact 终态、单一 Interactive pool、完整 output-lease 退休和进程共享 immutable device 策略保留，但它们尚未构成根因修复。
  ...
  - 临时阶段日志已删除；生产代码现以每 Worker 唯一的 `PreviewDecodeExecutionProgress` 固定原子快照区分 input/stream-info、hardware-device、codec-open、seek/flush、packet-read、codec-send/receive、materialization、output-lease 与 session-retire，并把快照纳入 Runtime/Headless/专业 Evidence。FFmpeg receive 同时严格区分 Frame、EAGAIN、EOF 和致命错误，致命 codec/device 错误不再被吞作“暂无帧”后复用污染 session。progress schema v2 再把 callback poll/cancel/request 证据纳入同一快照，accelerated native-surface endurance 也可写同类 journal；下一步先用压缩墙钟运行判定上述三分支，再决定修 request probe 或建立进程隔离/可恢复资源退休。在此之前不得加入按帧数轮换、交替 native pool 或把未返回 Broker lease 伪装完成的启发式修补。
