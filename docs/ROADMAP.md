...
本节是 2026-07-24 的代码基线，不是目标清单。状态区分“模型/算法存在”“已接入主路径”“已有真实项目证据”，避免从类型或单元测试推断产品完成度。
...
| 时间线编辑 | 多轨、移动、分割、普通 Trim、Ripple Delete、Overwrite、Roll/Slip/Slide、跨轨移动、锁定、吸附、多选和嵌套序列已有实现与测试。专业 Insert 已收敛为 UI 无关的原子 Sequence 操作：一次请求显式携带 target/ripple Track、精确间隔与 automation/Transition/navigation 策略；能跨轨开缝、切分跨界 Clip、保持完整链接组、拒绝锁轨/不一致链接、迁移安全转场并只在完整输入闭包下跟随 Bus/Output 自动化。Asset 产品 Action 在一个 Author Transaction 中注册音频 Scope、放置内容并提交一次 Undo；原局部碰撞逻辑已更名 `ClipOverlapMode::PushForward`。Clip 已用封闭 `ClipContent` 删除矛盾 kind payload，并增加 Sequence-local Basic Title；链接改为可承载任意成员的 `ClipLinkGroupId` 集合；复制/切割/覆写片段/序列复制会 fork 完整作者身份图；显式视频 Transition 已有强端点、精确范围、同轨不重叠不变量和不裁切的双源 handle demand；Cross Dissolve 产品命令会用媒体/嵌套真实范围默认拒绝短 handle，显式缩短与创建/删除均为单次 Undo 事务 | Track Targeting/Sync-Lock 的完整可视交互与工作区持久化、复杂 J/L linked-edge 决策、Lift/Extract、显式 Link/Unlink 产品 UI、reverse/freeze/time remap 仍未闭环；Cross Dissolve、Basic Title 和多轨 Insert 已有局部 Golden，但仍缺真实媒体/嵌套/独立视觉 reference 的顶层验收 | L1+ 作者、命令与基础 UI 闭环；专业 Insert 作者/Action/Golden 已闭合，交互表面待完善 |
...
机器可读合同身份是 `windows-alpha-golden-v5`，继续使用封闭 schema v3：未知字段失败关闭；Sequence 固定 3840×2160、25/1、square pixel、progressive、linear Rec.2020 working、Rec.709 legal Program Output、10-bit 交付默认值和 48 kHz stereo；H.264/AAC 与 HEVC Main10 绑定稳定内建 preset ID 及完整 signal/probe 合同。七个可组合切片都只通过普通产品接口执行：Foundation Audio 证明 PCM 导入及 Clip gain/pan/fades、Undo/Redo 和耐久重开；Editorial/Transport 证明 AAC、Overwrite、Split、Ripple、跨两条显式 ripple Track 的专业 Insert、Scrub、Accurate Seek 和 Synthetic Clock Master Play；Proxy/Relink 证明 canonical Proxy→Original→Proxy、结构化离线、同一 `AssetId`/Clip 重连及旧代理失效；Generated Delivery 证明 Transform/Opacity、H.264 High 与 HEVC Main10 真实导出、严格 probe 和普通媒体重导入；Visual Authoring 证明 Cross Dissolve、Basic Title、Hold/Linear/Bezier、工作空间感知 Primary Color 和显式 Rec.709 域 LUT；Recovery/Nesting 通过正式 Precompose Action、递归 Preview/Export、Autosave→新 Session 恢复→当前手动保存退役恢复点，固定父子 Sequence 内容与像素/执行证据；Color Media Roundtrip 导入项目生成的 HLG Main10 与 sRGB Alpha 静帧，以两条视频轨、静帧零速 hold、耐久重开、真实 Preview float-linear 合成、独立 sRGB→linear Rec.2020 数值检查、透明 RGB 不泄漏及 H.264 Rec.709 导出/重导入像素采样完成短窗闭环。生成画面、identity LUT 与 Solid Color 嵌套只提供回归一致性证据；HLG slice 当前只有编码码值和结构性工作空间检查，不替代完整绝对 HLG/PQ/Log 独立色彩 reference；短窗播放也不替代 30 分钟 CPAL/GPU/内存/A/V 门禁。顶层单 Project Coordinator 与三轮监督器已在本机连续通过；真实媒体嵌套色彩、恢复冲突/故障 UI、独立 HDR/Log reference 和合格发布机证据包仍未完成。

顶层 `GoldenAcceptancePlan` 把 fixture/operation/content/export 全部义务编译成确定性账本，并明确报告未规划与未绑定项；v5 的账本已经结构完整且不存在未绑定 fixture，但结构状态不观察任何工作流执行，`complete_golden_project` 仍固定为 false。七个独立切片同样不得声明完整 Golden；只有单一 Project 身份的总运行协调器能在完整观察后声明通过。专业 Insert 仍使用独立 target/ripple scope、链接组、锁定、Transition 和 Sequence-time automation 合同；`ClipOverlapMode::PushForward` 不得被重新解释为 Insert。Rust 与 PowerShell 校验器均按 schema v3、三轮要求和 slice export 引用失败关闭，不能由结构账本、单个切片、旧脚本或进程退出码虚构完整性。

单 Project 执行闭环现已收敛：`GoldenProductWorkflowDriver` 持有一个真实 App composition、固定 `ProjectId + project path` 与该运行的全部阶段 Sequence；Project-scoped 作者事务只要求 Generation 精确前进并允许活动 Sequence 改变，Sequence-scoped 事务还要求活动 Sequence 不变且其 Revision 精确前进。Foundation Audio、Visual Authoring、Editorial/Transport（含多轨 Insert）、Proxy/Relink、Generated Delivery、Recovery/Nesting 与 Color Media Roundtrip 七个切片均为可组合阶段；Recovery/Nesting 合法拥有父、子两条 Sequence，因此最终作者集合是八条而不是把“切片数”误作“Sequence 数”。专用 `mondrian-golden` 进程在同一 Project 中执行全部阶段，要求精确切片集合、切片报告只能为 partial、代理后台工作归零，并在最终 durable reopen 后逐条比较八份 Sequence 快照、Project/path、效果身份/参数、重连资产与代理意图、两个成品 profile、嵌套作者状态和色彩媒体作者状态；只有它能写出 `complete_golden_project: true`。PowerShell 监督器构建一次后执行三个不同 run/Project identity，按终态报告而非测试名、报告并集或退出码判定，三轮已在本机通过。完整独立 HLG/PQ/Log 数值 reference、真实媒体嵌套色彩、恢复故障/冲突 UI 与合格发布机 evidence bundle 仍明确阻塞。
...
- [x] 建立 archive/document/SQLite version registry，并提供 schema v21 current document fixture、v0 SQLite fixture、幂等打开、事务回滚和失败不覆盖测试；Alpha 不保留旧 document schema 兼容。v16 将 proxy membership 规范为有序集合；v17 冻结封闭 ClipContent、多成员 Link Group、强端点视频 Transition 和可持久化 Mask Property Bag；v18 增加完整持久化和校验的 Basic Title 封闭作者状态；v19 固定 Project 色彩环境、新 Sequence 模板和嵌套 Clip 色彩边所有权；v20 固定所有 Clip-owned 视觉处理共享的 Clip-local 作者时间；v21 分离 Sequence `color`/`delivery` 深结构并对 Project/Sequence 作者 JSON 启用未知字段失败关闭。
...
- [x] 建立版本化 Reference Corpus manifest、Golden/Stress Project 机器可读契约、Windows 参考机 profile、机器证据采集与资格验证、分层校验门禁；generated fixture 采用“固定 recipe、run-local artifact hash”而非伪造跨 encoder 位级稳定性，既有产物只有 attestation 同时匹配当前 recipe 与 artifact identity 才能复用，不能被新配方重新背书。`windows-playback-m0-v3` 将完整 Video+Audio gate、素材用途、环境绑定、45 分钟外部进程期限、Video 解码进度 journal、16 GiB baseline 内存档位和预期报告 profile 固化；编排器只允许清洁且首尾同 revision、完整门禁、合格机器和全部结构化报告通过的运行成为 baseline。`windows-alpha-golden-v5` 继续使用封闭 schema v3，固定完整 Sequence/交付合同、fixture purpose、slice export/window obligation 和“未执行不得通过”规则；Foundation Audio、Editorial/Transport、Proxy/Relink、Generated Delivery、Visual Authoring、Recovery/Nesting 与 Color Media Roundtrip 七个切片分别执行真实产品工作流，证据是作者事务、源/代理指纹、媒体 probe、结构化不可用、持久化、交付 probe、恢复身份、静帧 hold 与像素/语义事实而非字符串清单。App Action 不再允许未知/未实现意图静默成功。低于门禁档位只有在所有资格问题均被版本化合同显式列为可诊断、且操作员显式 opt-in 时才可继续，结果必为 diagnostic；缺 GPU/工具链/系统等执行前提仍失败关闭。三轮完整工作流已有本地结构化证据；合格发布机 capture 与 release evidence bundle 仍按 M1/M2 门槛单独验收。
- [x] Golden v5 的结构完整性已有单一深 Module：确定性计划账本逐类计算必需、已规划、缺失与未绑定义务；Rust/PowerShell 同步校验 schema、三轮要求和 slice export 引用；当前账本结构完整，所有独立切片仍无论成功或失败都显式声明 `complete_golden_project: false`。结构账本只关闭“计划无遗漏且不会误报”；完整执行由独立的单 Project Coordinator 观察，不允许结构状态或切片报告越权声明完成。
- [x] Golden 单 Project 顶层 Coordinator 已进入专用进程主生命周期：Headless driver 固定一个 `ProjectId + path + AppState`，区分 Project/Sequence 两类作者事务证据，并在初次 open、阶段 Sequence 创建、Autosave Recovery 和 durable reopen 前后失败关闭身份漂移；七个可复用切片在同一项目内执行两次代理生成、离线重连、两种交付与重导入、正式 Precompose、恢复及退役、HLG/Alpha 导入与短窗导出。最终 reopen 对照八条 stage-owned Sequence 的完整序列化快照、同一 relinked Asset/path/proxy intent、H264High/HevcMain10 成品、父子 Sequence 图和色彩媒体作者状态；代理队列/运行/失败必须归零。三轮监督器只接受精确顶层 schema、不同 run/Project identity 和每轮唯一的 `complete_golden_project: true`，已在本机连续通过；完整独立 HDR/Log reference 与合格发布机 evidence bundle 仍须完成。
...
- schema v21 current fixture 可保存、重开；Project 色彩环境/新 Sequence 模板、封闭 Sequence `color`/`delivery`、Clip-local 视觉作者时间、Basic Title/Mask 动画与强编辑关系 round-trip；旧/未来 schema 与未知作者字段明确拒绝；故意失败不会破坏源文件。
...
- [ ] Bin、重命名、缩略图、离线占位、单个与目录重连可完成 Golden Project。单素材 `proxy-relink-v1` 已证明离线诊断、显式 Relink、`AssetId`/Clip/名称保持、Asset Library revision、reload event、replacement source 解析，以及源路径参与代理身份后旧代理不被误用并重新生成；顶层三轮已重复该单素材路径，但目录批量重连、运行中 waveform/thumbnail/decoded cache 组合失效、多个素材/嵌套压力和离线占位交互仍未补齐，因此本项不提前勾选。
- [ ] durable worker、SQLite snapshot、Session/generation completion、dirty/manual/autosave baseline 和深 Recovery Module 已完成；fixture-free Golden 已覆盖正式 Precompose 后 Autosave、关闭原 Session、产品恢复 Action、恢复点保留、当前手动保存退役和再次重开，且父子 Sequence/Preview 像素/Export 执行不漂移，顶层三轮也已重复该正常恢复路径。仍须补齐恢复来源/时间/目标/冲突 UI、磁盘满/权限失败注入、反复崩溃和 Save As/close 压力边界。
...
- [ ] CPU decoded cache、GPU/working cache、proxy index 都有字节预算、LRU/eviction、source revision 和颜色解释 key；整进程 Private Commit v1 已用 canonical 生成素材在 M0 固定参考机完成 Video+CPAL/A/V 基线。`proxy-relink-v1` 已在隔离的项目 cache root 真实生成 H.264 proxy、用 canonical Preview resolver 完成 Proxy→Original→Proxy，并在 relink 后证明新路径为 Missing 而非复用旧代理、随后发布新鲜 replacement proxy；仍须多个素材/嵌套项目、cache eviction/内存压力、8 GiB 最低档和更多 GPU/驱动的组合矩阵。
- [ ] 播放开始后音频保持主时钟；视频迟到采用 drop/repeat/降质，不能把常态播放变成反复静音等待视频。

### 音频最低闭环

- [ ] Clip gain、pan、fade in/out 已贯通稳定 Edit ID 的产品 UI、单字段类型化命令、完整作者校验、细粒度 Undo、保存重开和 scalar/SIMD 公共执行；0 fade 规范为 `None`，非法时长原子拒绝。规范 PCM 的 Headless Golden foundation 切片现已真实执行 -6 dB、+0.25 pan、双侧 1 秒 equal-power fade、四次 Undo/Redo、耐久保存重开和稳定 ID 后置条件，并进入三轮顶层 Golden；完整 Rack/Automation 编辑 UI、更多布局和真实插件路径仍未闭合，因此本项保持未完成。
...
- [ ] 产品 UI 已完成 H.264/AAC SDR MP4 与 HEVC Main10 的稳定合法预设、类型化 profile/位深/range/chroma/Alpha 合同、统一兼容性阻塞、队列、取消和失败原因；Golden v5 的两条交付合同已绑定稳定内建 preset ID，普通 CI 会逐项对照真实 `resolve_export_delivery` 与共享 `expected_export_video_signal`，防止 8/10-bit、profile、chroma、pixel format、range、Rec.709 CICP、静态 HDR metadata 缺席策略、Alpha、分辨率和 AAC 码率漂移。生产队列现在以 `Required/Forbidden` 类型化流合同在发布 `Completed` 前精确 probe mux/major brand、codec/profile、位深、rational fps、画幅、pixel format、CICP/range、静态 HDR metadata 精确值或缺席、音频 codec/sample rate/channel layout 与时长；不可变导出依赖冻结源画幅，媒体/嵌套/Transition/生成层统一把作者 Transform 投影到实际交付采样画幅，并将目标尺寸纳入 decode cache identity；25 帧 generated-delivery slice 已真实完成 H.264/HEVC 导出和普通媒体重导入，Color Media slice 又以 4K 作者→1080p 交付验证构图没有重复缩放，两者均进入三轮顶层 Golden。当前全部已实现 preset 参数已进入同一可编辑草稿和产品表单：容器、codec/profile、跟随序列或显式画幅、位深/range、chroma、Alpha、CRF/完整 VBV、GIF palette/dither、禁用/AAC/MP3/PCM 及其参数；选择内建 preset 会精确重置，非法中间组合保留供继续编辑但无法构造入队动作。仍需覆盖确认、失败重试、完整独立色彩 reference、长项目与合格发布机证据；不能因当前短窗合同通过而勾选整项。
...
- [ ] 导出后自动 probe 已进入生产完成态，generated-delivery slice 也已真实重导入并校验 MP4、codec/profile、分辨率、rational fps、位深/pixel format、时长、48 kHz Stereo AAC、primaries/transfer/matrix/range 与静态 HDR metadata 缺席并进入三轮顶层 Golden；仍须在独立真实色彩 reference、长项目和其他已支持容器/音频布局上关闭此项。

### 退出门槛

- [x] Golden Project 完整工作流由一个监督命令连续 3 次通过；三轮使用不同 run/Project identity，无人工修复项目文件或清缓存步骤。
...
- 落地项目 version registry 与 schema v21 current fixture；Alpha 旧 schema 明确拒绝。
...
- 已建立一次命令连续运行三轮的 Golden Project 监督器并完成当前三轮；每个候选构建仍必须重跑且修复全部 P0。
