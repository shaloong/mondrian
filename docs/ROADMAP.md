...
本节是 2026-07-20 的代码基线，不是目标清单。状态区分“模型/算法存在”“已接入主路径”“已有真实项目证据”，避免从类型或单元测试推断产品完成度。
...
| 项目持久化 | `.mdp` manifest、schema v15 精确时间/跨媒体参数/Sequence author revision/统一时间显示设置/规范音频信号布局/Component 混音矩阵/可自动化 Route 边/精确 Samples 参数单位文档、当前 fixture、SQLite 素材库、临时文件写入、自动保存、恢复候选和重连已接入 | Alpha 明确拒绝旧 schema、不承诺兼容迁移；替换目标文件前的持久化/崩溃语义仍需压力验证 | L1- |
...
| 效果与动画 | 稳定 `EffectId` 与 `ParameterId`、实例地址、版本化 `ParameterSchema`、定义默认值/可动画能力、受约束数值/enum/resource、三种执行插值语义、`PropertyBag`/`AnimatedProperty`、编辑器曲线预设、效果 DAG、mask、缓存策略和插件式 definition/DSL 已存在；视觉与音频 Processor 共用同一参数描述语言，项目加载会拒绝非法参数状态，内建执行按 ParameterId 精确寻址；效果库只列出有可执行图的定义，启用但未实现/缺插件/资源无效/构图失败会在共享 Render Plan 编译前结构化失败，不再静默恒等输出 | 只有部分声明效果生成真实 render op；CPU/GPU backend 与真实 preview/export 验证仍须分别声明；文字和转场类型尚未接入时间线/渲染主路径；真实外部插件仍需 Adapter | L1- 地基，产品广度未完成 |
| 音频 | Track→Clip 已是 placement SSOT；Sequence 作者层统一持有 Component/Scope、Track/Bus/Output、typed Route/send、Rack、Automation 与 Transition；播放、导出、Audition、Analysis 和嵌套只允许共用 `compile → prepare → Session → Runtime`。稠密 schedule 已拥有拓扑 slot、连续 Route/Contribution ranges、精确 source/causal-execution/automation spans、liveness scratch、PDC 和 scalar/SIMD；callback 不扫描作者图、不分配、不隐式 clipping。schema v15 支持规范信号布局、显式 Component matrix、可自动化 Route 和精确 Samples 参数单位。Processor semantic IR 在 prepare 期由 Resolver 绑定固定 mode/algorithmic-latency/tail/state/scratch Factory，Render Contract 显式限制 Processor 私有字节、公共输出 lookahead 与 PDC delay-line 字节，Session 构造复核总量；`built_in_processors` 独立拥有规范 Gain 与状态型 Sample Delay，二者共用 Host 与预分配 Parameter-ID batch。算法延迟与 `None/Finite/Infinite` tail 已分离；Contribution 会在源结束后以显式静音冲刷有限尾音，并在首次真实相交样本懒进入局部状态。每个 Processor/Scope/Edit/Fader/Route 按自身 input-signal delay 求值自动化；Program Output 在 Session 内预热并丢弃固定 lookahead，直接返回 Timeline-aligned PCM，嵌套 child 不再向父层泄漏或重复补偿 latency。非零延迟测试 Adapter 覆盖 PDC/entry/分块/mode/poisoning/stage-time/公开输出对齐/嵌套/预算；Sample Delay 覆盖整数作者值、可听延迟不被 PDC 抵消、clip tail、未来 clip 懒激活、seek 重入、分块和预算拒绝。Program Output 已有 callback 无分配 sample-peak/RMS/clip/non-finite observation；本地真实 CPAL/A/V 30 分钟同步门禁已通过 | 仍需隔离 VST3/CLAP、完整 matrix 编辑/探测/Adapter 布局协商、首个生产级非零算法延迟 Processor、stateful reverse checkpoint/materialization、typed sidechain、true-peak/响度/limiter/ballistics UI、完整 Rack/Automation 编辑 UI/Undo、固定参考设备矩阵与声学 loopback。GPU 仅能作为固定批量延迟已计入 PDC、deadline 有证据且可控降级的可选 Processor backend，不能成为 callback 依赖 | L1+ 执行地基；不宣称 DAW 完成度 |
...
- [x] 建立 archive/document/SQLite version registry，并提供 schema v15 current document fixture、v0 SQLite fixture、幂等打开、事务回滚和失败不覆盖测试；Alpha 不保留旧 document schema 兼容。
...
- [ ] FFmpeg open/stream-info/seek/packet I/O 已接入 request-scoped interrupt，取消结果会保留首次观测 checkpoint 与 cooperative/FFmpeg-I/O 机制，外部 still 子进程可 kill/wait/join；loopback HTTP stall 已在 media Interface 与生产 Preview worker 两层证明 blocked input-open 会由真实 `AVIOInterruptCB` 有界返回并进入 Runtime/专业报告聚合，而不是只声明回调。播放域已统一 5 ms request→checkpoint、50 ms Playback/Interactive return、500 ms Still return 的 fail-closed 门禁。规范 corpus 已提供不承担色彩正确性声明的确定性 4K25 HEVC Main10 Long-GOP 配方，Reference Playback Run 会绑定 recipe/artifact/机器/报告身份；仍须在合格固定参考机用 pause/seek/close/quit、真实驱动以及 stream-info/seek/packet-I/O 或驱动阻塞取得最坏延迟证据，且 UI 线程不得 join worker。
- [ ] `ProcessMemoryProbe` 已建立无 OS 调用的接口与 Windows Process Status Adapter；专业门禁以固定 cadence 聚合 5–10/25–30 分钟 Private Commit 稳定窗、4 GiB 绝对上限和各门禁声明的 terminal stress 静止后收敛，unsupported/缺样/探针错误失败关闭；视频 v4 还要求主视频流时长及声明帧数覆盖，且专业 cadence/timeout/latency/readiness/hardware 阈值不读取 smoke 调参。原生 `AVFrame` 可能钉住整个硬件 surface pool，现已在停止传输、最终注册 GPU Viewer 输出精确绑定当前完整 Preview generation 且 Broker/执行意图静止后，仅释放 media payload/pin；renderer copy fence 独立证明源 lease 退休，worker idle 或已确认的驻留族切换销毁 FFmpeg Session。GPU Viewer 输出、stale pin 与失败记忆继续有效，后续同 generation 查询不会自激重解码；generation 旋转前会在旧输出仍具 exact 证明时重试释放，旋转后 stale presentation 不再有权授权源驻留决策。准确 seek 的 Long-GOP 非参考前缀优化恢复完整解码后才允许选帧，submitted packet 同样计入预算，专业门禁要求 temporal approximation 为零。2026-07-20 的清洁 revision Reference Run 中，真实 CPAL/A/V 30 分钟门禁通过（零 drift/underrun/recovery，整进程 settled growth 约 85.6 MiB），但视频在完整 30 分钟连续播放后进入 seek 时暴露 D3D12VA still 会话在 codec 内不可协作阻塞；逐请求销毁/重开 decoder 仍在第二个精确 still 阻塞，证明该临时策略不是根因并已删除。当前生产拓扑最多两名 Preview worker：`Playback` 与共享 `NonPlayback`；三种访问语义仍独立决定 seek、精度、预算和取消策略，但物理驻留只分互斥的 Playback/Interactive 两族。Scrub 与 GPU exact Still 共享一个 Interactive session，CPU Still 保留独立非 native slot；切换族时先清 native Frame Store、通过 Broker revision 唤醒 owner，并在相反 worker 销毁其上下文并确认前拒绝新族准入。共享 Interactive session 只有在上一 native output 的 Frame Store/renderer lease 全部退休后才可 seek/flush，不创建备用硬解 pool；源/指纹/设备/几何/色彩契约变化先销毁旧 session，每个 exact 随机目标仍执行索引化 seek + codec flush，取消部分输出不得缓存。FFmpeg 设备级身份现按 backend+renderer adapter 在进程内共享，每个 codec 独立持有 `AVBufferRef`，而 codec/DPB/`AVHWFramesContext`/surface pool 仍按驻留族销毁；这移除了 Playback→Interactive 切换中的驱动 device teardown/recreate 竞态但没有扩大硬解驻留。2026-07-21 的真实 4K Main10/100-seek release 短门禁完成 300/300 Ready、全程 D3D12VA P010 原生 GPU、零上传/回读/回退；Playback 1 open/79 reuse、Scrub 1/49、Exact 1/51，52 个 exact 结果零近似，99 次 superseded 取消的 request→checkpoint 最大 364 µs，跨区 seek 约 11.2 秒。终态 pending/queued/in-flight、media entries、native resource units 与 media pin 全部归零，峰值 Private Commit 约 1.24 GiB。定向诊断还证明全新 device 在第 44,994 帧 exact 可于约 239 ms 完成，媒体层连续解码 44,994 帧并销毁/重建 codec 后的 exact 可返回。随后清洁 revision `f77ce37` 的正式 Video Run `20260721T132057Z-local-windows-dev-01-11bb6263` 在真实 cadence 约第 18,879 个 Playback 成功帧后进入未返回的 codec 调用：完成泵已收割全部 19,198 个既有结果且从未耗尽 8 项单轮预算，因而不能归因为 UI 完成积压；同一生产闭环压缩墙钟后又成功解码 20,004 帧、真实 GPU 注册 20,002 帧并完成 Playback→Interactive 退休，排除了固定帧数或单纯累计 surface 泄漏。当前修正把 App 完成通道固定为 8 项、背压时保持 shutdown/residency 可中断并终结 Broker 绑定；FFmpeg 取消后继续复用已分配 session/device，但统一 flush codec、清 session ring/PTS/EOF 并强制下一请求索引 seek，不再沿用可能只提交或只排空一部分的 codec 位置。确定性 1812 秒视频与 1835 秒音频配方、机器资格验证、完整 Video+Audio 编排及 evidence bundle 已闭合；仍须在该修正的清洁 revision 上完整复验 30 分钟 Video 门禁，只有 cadence、取消、100 次 seek、终态驻留和内存全部通过后才关闭本项。

  - 后续清洁 revision `4b57403` 的正式 Video Run `20260721T151752Z-local-windows-dev-01-e0c128d5` 已完成全部 44,994 个 cadence Playback 帧且保持原生 D3D12VA P010，但首个 exact 返回后第二个 exact 再次停在 codec 内；另一个压缩墙钟生产闭环连续完成 45,000 帧并追加 2 次 seek，证明累计帧数或设备存活本身不是充分条件。当前策略因此把成功的 native exact 定义为共享 Interactive codec 的终态：已释放的 Scrub session 可承接一次 exact，但结果 lease 退休后必须在下一份 Broker execution lease 之前销毁 codec/DPB/frames context；不开放交替 native pool，进程共享 immutable device 与 seek index 继续复用，驻留族 ACK 也必须等待所有已发布 native lease。2026-07-21 的真实 4K25 Main10 短诊断完成 60/60 Ready、100 次跨区 seek、162/162 D3D12VA P010 原生 GPU 帧、零上传/回读/回退，seek 总计约 15.45 秒；取消 request→checkpoint 最大 15 µs、终态 native resource units 为零、峰值 Private Commit 约 1.37 GiB。该短诊断的通用 50 ms 慢帧分析仍会把 Long-GOP exact 和一个 56.6 ms cold Playback 帧标为性能风险，不能替代专业 profile；完整 30 分钟 Video 门禁仍待在清洁 revision 上复验。
...
- schema v15 current fixture 可保存、重开；旧/未来 schema 明确拒绝；故意失败不会破坏源文件。
...
- 落地项目 version registry 与 schema v15 current fixture；Alpha 旧 schema 明确拒绝。
