...
- [x] `PlaybackEngine`、`FrameWorkBroker`、`PreviewFrameStore`、Monotonic Runtime Clock、Playback/Frame Cancellation Evidence 与 Headless GPU Adapter 已从 UI 收敛；Broker execution lease 是优先级/访问类/worker lane 驻留证据的单一事实来源，App activity 原子计数已删除；访问模式/Broker Adapter/有界 job transport 位于 `app::preview_access_mode`，Window 与 Headless 的“单次 Demand 采样→完成/过期→终态 Delivery→预卷”统一在 `app::playback_preview`；预览内部的 timeline evaluation、media adaptation、Viewer plan、media execution、presentation 与 diagnostics 已按行为所有权成为私有深 Module，最终 GPU/Raster/stale/CPU output 仲裁只在 presentation Module，且未扩大跨层 Interface。
