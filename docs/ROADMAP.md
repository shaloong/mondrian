...
`GoldenProductWorkflowDriver` 仍持有一个真实 App composition、固定 `ProjectId + project path`，并使七个切片可复用为诊断阶段；当前实现仍产生八条 stage-owned Sequence（Recovery/Nesting 包含父子两条）。此前三轮运行真实证明了这些局部路径和共同 Project 生命周期，但没有证明它们在一条 5 分钟节目时间线上共同成立，因此该三轮不再计作 M1 完整 Golden。v6 现在会在重载荷执行前明确列出 Hero 缺失义务并失败关闭。下一步不是删除切片，而是让素材编辑、视觉、音频、颜色、嵌套和交付逐步复用一条稳定 Hero Sequence；最终 durable reopen 必须保留该身份、7,500 帧内容边界及全部关联证据。
...
- [x] 建立版本化 Reference Corpus manifest、Golden/Stress Project 机器可读契约、Windows 参考机 profile、机器证据采集与资格验证、分层校验门禁；generated fixture 采用“固定 recipe、run-local artifact hash”而非伪造跨 encoder 位级稳定性，既有产物只有 attestation 同时匹配当前 recipe 与 artifact identity 才能复用，不能被新配方重新背书。`windows-playback-m0-v3` 固化完整 Video+Audio gate、素材用途、环境绑定、45 分钟外部进程期限、Video 解码进度 journal、16 GiB baseline 内存档位和预期报告 profile。`windows-alpha-golden-v6` 使用封闭 schema v4，固定 Hero Sequence、完整 Sequence/交付合同、fixture purpose、slice role/export/window obligation 和“未执行不得通过”规则。七个局部切片的作者事务、源/代理指纹、媒体 probe、结构化不可用、持久化、交付 probe、恢复身份、静帧 hold 与像素/语义证据仍有效；它们不再被误写成同一节目闭环。App Action 不允许未知/未实现意图静默成功；缺 GPU/工具链/系统等执行前提仍失败关闭。
- [x] Golden v6 的结构完整性由单一深 Module 负责：确定性全局账本与 Hero 账本分别计算必需、已规划、缺失与未绑定义务；Rust/PowerShell 同步校验 schema、Sequence role、Hero 身份、三轮要求和 slice export 引用。所有独立切片无论成功或失败都显式声明 `complete_golden_project: false`；当前全局账本完整，但 Hero 账本明确 blocked，这正是防止“多个孤岛相加等于成片”的预期结果。
- [x] Golden 单 Project Coordinator 和监督器的机制已进入专用进程主生命周期：Headless driver 固定 `ProjectId + path + AppState`，区分 Project/Sequence 两类作者事务证据，并在 open、阶段 Sequence 创建、Autosave Recovery 和 durable reopen 前后失败关闭身份漂移。Coordinator 现还要求全部 Hero slice 的 primary `SequenceId` 唯一一致、最终 reopen 保留 Hero 身份且内容边界精确为 7,500 帧；stage-owned Sequence 可共享身份，嵌套 child 可额外存在。当前阶段尚未迁移到同一 Hero，因此监督器应失败而不是复用此前隔离 Sequence 三轮的 `complete=true` 结果。
...
- [ ] Clip gain、pan、fade in/out 已贯通稳定 Edit ID 的产品 UI、单字段类型化命令、完整作者校验、细粒度 Undo、保存重开和 scalar/SIMD 公共执行；0 fade 规范为 `None`，非法时长原子拒绝。规范 PCM 的 Headless Golden foundation 切片现已真实执行 -6 dB、+0.25 pan、双侧 1 秒 equal-power fade、四次 Undo/Redo、耐久保存重开和稳定 ID 后置条件，并在旧隔离 Sequence 三轮组合运行中重复通过；完整 Rack/Automation 编辑 UI、更多布局、Hero Sequence 共存和真实插件路径仍未闭合，因此本项保持未完成。
