...
| 渲染与色彩 | Preview/Export 共用按 Sequence/Registry revision 与作者 fingerprint 绑定的不可变 `PreparedVisualProgram`；逐帧 `PreparedVisualFrameClosure` 固定 nested time、canvas/color、Transition/temporal binding 与 instance path，区间 `PreparedVisualRangeClosure` 固定 selected-range 的媒体、Transition 和字体依赖。`CompiledEffectGraph` 是唯一生产 IR，动态 topology/cache 只驻留于调用方 `EffectExecutionSession`。有限历史 CPU-F32 tracer 已执行精确输入 ROI，并用编译 schedule/use-count 驱动 current-time unary fan-out、Blend/MultiInput join 与精确 live-set：last-use move、必要时 clone、join 即释放，边消费或 owned-byte ledger 不闭合则失败；Blur/Vignette/Grain 及含 Dissolve 的 temporal DAG 与完整 reference crop 数值对等。异构提交开始后的取消、GPU 或证据失败是本次 attempt 终态，不隐式整图 CPU 重跑 | Temporal 与异构仍是 bounded tracer：历史算子上游 Effect、Mask ROI raster、future/stateful、完整画面自动切片、temporal×heterogeneous 和 GPU parity 未闭合；Preview/Export 的完整画面交付也通常请求整帧，不能把局部执行器能力冒充产品级自动切片。后续必须以生产有限历史/扩大 ROI Processor、嵌套和更多 placement 为输入，证明 exact window、halo/full-frame/unknown、continuity、resource lifetime、seek/cancel 与 CPU/GPU reference parity；缺少 exact mode、live-set 或 Adapter 时在像素执行前阻止。Log/HDR 仍需独立 reference | L1+ 唯一 IR、Prepared closure、精确 ROI 与 bounded temporal DAG scalar 边界闭合；通用 Temporal/ROI/异构执行未完成 |
...
- 已让 CPU-F32 scalar executor 消费同一 demand：精确输入 tile 保留完整画布坐标，有限核消费实现推导 halo，full-frame/unknown 证据不降级；编译 schedule/use-count 的纯 dry-run 与执行共同驱动 last-use move、fan-out clone 和 Blend/MultiInput join 释放。需求批次在物化前给出精确 Float32 source coverage，Preview/Export 先做 CPU grant 准入；Effect Session 再把 retained coverage、resident value、kernel scratch、唯一 final output 和单 tile live-set 纳入同一硬上限。完整请求不能直接容纳时，Session 自动生成确定性、非重叠、最多 4,096 块的二维计划，复用已冻结 coverage，取消或任何计划/账本失败都不发布半帧或局部缓存；最终 `Vec` 进入 `Arc<Vec<_>>` 不复制整帧像素。Blur/Vignette/Grain 以及有限历史→current-time fan-out/join DAG（含坐标种子 Dissolve）已有 direct/tiled production 与 reference parity；allocation-free UHD 门禁证明两帧 pixel-local 请求在标准 384 MiB grant 下形成 64 个 `480x270` tile，逻辑峰值 402,278,400 bytes。下一步是不改变该语义参考地补真实 4K 进程内存/吞吐 evidence、Mask 的 coordinate-aware cancellable raster、effected history upstream、future/stateful continuity、temporal×heterogeneous 与 CPU/GPU parity；不得用错误裁剪像素或隐式整图 fallback 换取性能。
