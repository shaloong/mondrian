...
| `mondrian-app/src/app/legacy_egui_preferences.rs` | Reference quarantine | Legacy egui preference and new-project draft schema has been moved out of `app/mod.rs` into a restricted `app`-visible module. It still compiles for the eframe reference app, while self-hosted product preferences live in `self_hosted::preferences_store`. | Delete with the legacy eframe app; do not add new product preference fields here. |
