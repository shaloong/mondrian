...
**Prepared Heterogeneous Effect Work**:
The UI-independent executable CPU-Float32-prefix/GPU-Float32-tail seam prepared from one Effect Graph Value Execution Plan. It accepts an already materialized caller-owned working frame, executes the exact scalar CPU prefix, and returns pixels plus completed CPU-token evidence and the exact pending transfer/fused-GPU continuation. Once that completion is accepted, a caller must submit or terminally fail the same heterogeneous attempt; the seam never silently reinterprets the whole graph on CPU.
_Avoid_: CPU work in Viewer/UI lowering, GPU capability probe presented as execution, suffix failure followed by hidden whole-graph CPU fallback, completed upload/output token before Adapter evidence
...
- **Prepared Heterogeneous Effect Work** currently supports exactly the real Gaussian CPU-Float32 prefix to Basic Correction/Grain GPU-Float32 tail tracer when the caller already owns the working frame. Preview integrates that bounded tracer for Media input batches through its dedicated visual-execution worker and move-only completion lease. Export integrates it for Media, Basic Title, and same-raster Nested Sequence placements through an attempt-local `ExportVisualRenderSession` that performs upload, exact GPU dispatch, wait, and readback before ordinary compositing; Solid Color, Adjustment, and Transition input retain a complete CPU route or fail closed. Both consumers retain generation, cancellation, and terminal Adapter evidence. This production vertical slice does not prove general heterogeneous DAG execution, arbitrary placements, color-domain conversion, external lanes, or temporal-plus-heterogeneous execution. Viewer lowering may not synchronously manufacture the CPU prefix, and a started heterogeneous attempt may not fall back implicitly to whole-graph CPU execution.
