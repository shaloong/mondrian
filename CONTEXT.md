...
- A professional **Insert Edit** is a Sequence edit over explicit target Tracks and Sync-Lock/ripple scope, not the local `ClipOverlapMode::Insert` collision resolver. It must split material at the insertion boundary, preserve linked-group consistency, respect locks, and migrate or reject affected Transitions and Sequence-time automation atomically. A single-Track push-forward operation cannot satisfy the Insert Golden obligation.
