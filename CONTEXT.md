...
- A **Prepared Visual Schedule** is the production visual-evaluation representation shared by Preview and Export. Its cache key is exactly Sequence identity plus **Sequence Author Revision**; preparation validates before publication, cache failure cannot replace a valid entry, and interval selection must remain semantically identical to the direct Sequence reference Adapter at Clip and Transition edges.
