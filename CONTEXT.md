...
A lock-free, single-writer evidence snapshot for one Preview media worker: the exact current FFmpeg/media stage, a request sequence, and a stage-publication sequence. It is observable by the UI-independent Preview Production Runtime and Headless acceptance but owns no timeout, cancellation, recovery, or scheduling decision.
_Avoid_: Debug log as liveness proof, coarse `codec` bucket, watchdog hidden in the media Adapter, progress sequence treated as completed work, UI-owned worker health
