...
The explicit worker-owned media execution context for FFmpeg input/codec state, DPB, and hardware-surface pools. Playback and Scrub have independent continuous slots; CPU Still has one physically separate slot; GPU-resident exact Still has a fixed two-slot ring. Every native output carries a weakly observed lease through App and renderer clones. Exact work first reuses an existing released session and opens the spare slot only while the existing output remains leased; it never opens another hardware pool merely to alternate slots. If both GPU Still slots are leased, the worker waits at a cancellable, evidenced backpressure point. Idle and worker shutdown clear the complete context.
