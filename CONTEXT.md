...
- `app::preview_scheduler_policy` is the sole application policy for Frame Demand deadline classification, executed decode quality, bounded playback prefetch depth, and the consecutive-late decode-pressure guard. That local guard may suppress speculative or duplicate decode work but cannot enter Playback Transport `Recovering`, change Clock Master, or mutate authored quality. Preview UI Adapters consume these decisions but cannot redefine `Ready`, `Degraded`, or `Late`.
