...
**Finite Temporal Tap Program**:
The private execution projection of one admitted `CompiledEffectGraph` into its ordered Source-fed temporal taps. Each tap carries an exact signed `sample_offset`: negative requests history, positive requests lookahead, and zero reuses current input. Demand collection preserves compiled order while de-duplicating exact sample times. Scalar dry-run and execution share per-offset use counts so one frozen sample remains charged and resident until its last tap, then retires exactly once. A tap with an upstream Effect value, unbounded demand, or continuity state is outside this bounded program and fails closed.
_Avoid_: Past-only offset type, one fetch per node for duplicate time, releasing a shared sample before last use, separate Preview/Export temporal IR, evaluating output-time-bound upstream parameters at another sample time
...
- Finite Temporal/ROI scalar execution consumes the same `CompiledEffectGraph` through an exact request carrying Clip visual-domain **Timeline Time**, the ordinary Render Plan's output-frame seed, complete frame coordinates, generation, cancellation, ROI evidence, and Float32 precision. The admitted shape is a **Finite Temporal Tap Program** of Source-fed signed past/future taps plus a current-time unary/fan-out/join DAG, including built-in Mask/MaskSource. Exact duplicate times are demanded and frozen once; shared dry-run/runtime use counts keep each sample resident and charged until its last tap. Mask geometry is prepared once per evaluated graph and frame extent, uses exact global pixel centers for every region, and charges retained geometry plus maximum row scratch to the same hard grant. Temporal taps with upstream Effects, unbounded input, stateful continuity, unresolved domains, and unsupported operations still fail closed. A demand batch reports exact Float32 source-coverage bytes before materialization. Preview and Export admit that coverage against their CPU working-set grant, freeze it under generation cancellation, and expose only bounded subregion copies with no decode, conversion, nested evaluation, or allocation at the Effect provider Seam. The Effect Session dry-runs the compiled schedule/use counts, includes retained coverage, prepared Mask resources, graph live values, temporal sample liveness, kernel scratch, one final output and tile work in its hard grant, executes directly when possible, or derives a deterministic non-overlapping 2D schedule capped at 4,096 tiles. The final `Vec` transfers into shared cache/output ownership without a full-frame pixel copy; internal tiles are attempt-local and cannot read or publish output-cache entries, so cancellation or any planning/ledger failure publishes neither a partial frame nor partial cache state. Export uses its exact job-local closure provider. Preview submits all media temporal demands through its asynchronous Scheduler, binds CPU-working representation into request/cache identity, returns typed Temporal Pending while any Frame Store value is absent, and never blocks for decode or substitutes the displayed/current frame. Direct and tiled Preview/Export production execution share this contract; GPU temporal tiling and the remaining blocked shapes remain unclaimed.
