...
- `app::preview_scheduler_policy` is the sole application policy for Frame Demand deadline classification, executed decode quality, and bounded playback prefetch depth. Preview UI Adapters consume these decisions but cannot redefine `Ready`, `Degraded`, or `Late`.
