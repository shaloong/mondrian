...
The renderer-owned immutable CPU-Float32-prefix/GPU-Float32-tail authority prepared from one Effect Graph Value Execution Plan before source pixels are materialized. It freezes compiled-graph identity, exact extent, graph-planning budget, CPU prefix, explicit transfer, and fused GPU suffix. Preview and Export later bind a typed caller-owned working frame only when extent and resource authority still match; the worker executes the prepared work without replanning. The CPU completion carries completed CPU-token evidence and the exact pending transfer/GPU continuation. Once accepted, a caller must submit or terminally fail the same attempt; the seam never silently reinterprets the whole graph on CPU.
_Avoid_: Route choice after decode, CPU work in Viewer/UI lowering, worker-side replanning, GPU capability probe presented as execution, changed extent or budget accepted after preparation, suffix failure followed by hidden whole-graph CPU fallback, completed upload/output token before Adapter evidence
