...
- Finite-history/ROI scalar execution consumes the same `CompiledEffectGraph` through an exact provider request carrying Clip visual-domain **Timeline Time**, the ordinary Render Plan's output-frame seed, complete frame coordinates, generation, cancellation, ROI evidence, and Float32 precision. The provider identity covers the complete source/nested mapping and pixel interpretation. Unknown ROI requests remain conservative full-frame work without exact-halo evidence; stateful, continuity-owned, unbounded, unresolved-domain, or unsupported graph semantics fail closed. Export supplies an exact job-local provider for the declared past-only, stateless CPU-Float32 `Source -> TemporalFrameMix -> current-time unary tail` tracer. Preview submits every exact media history demand through its existing asynchronous Scheduler under the current Preview generation, binds CPU-working representation into request/cache identity, returns typed Temporal Pending while any Frame Store value is absent, and executes only after freezing the complete set under the generation cancellation token. It never blocks for decode or substitutes the displayed/current frame. Other temporal/continuity/ROI shapes still fail closed.
